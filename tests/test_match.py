...
def test_owner_qualified_controller_aliases_do_not_collide() -> None:
    symbols = tuple(
        ...
        for address, name, alias in (
            ...
            (0x4407A0, "update_pause_menu", "?AI@cRSubPause@@QAEXXZ"),
            (0x43EE50, "update_track_speedup", "?AI@cRSubSpeedUp@@QAEXXZ"),
            (0x443130, "update_track_parcels", "?AI@cRParcelManager@@QAEXXZ"),
            (0x4431D0, "update_track_parcel", "?AI@cRParcel@@QAEXXZ"),
            (0x4425F0, "update_vapour", "?AI@cRVapour@@QAEXXZ"),
...
def test_masked_operand_audit_accepts_aligned_shifted_jump_table_targets() -> None:
    mapped = bytearray(b"\x00" * 0x3000)
    struct.pack_into("<II", mapped, 0x2000, 0x401007, 0x40100C)
    target = bytes.fromhex("ff24850020400031c08bc8c3b801000000c3")
    candidate_code = bytes.fromhex(
        "ff2485000000008bc831c0c390b801000000c3"
    )
    candidate = ObjectFunction(
        name="_foo",
        data=candidate_code + (b"\x00" * 8),
        relocation_offsets=frozenset({3, 19, 23}),
        relocation_references=(
            ObjectRelocationReference(
                offset=3,
                symbol_name="$Ltable",
    ...
    entry = result.masked_operand_audit.entries[0]
    assert entry.target_references[0].jump_table_entries == (7, 12)
    assert entry.candidate_references[0].jump_table_entries == (7, 13)
    assert entry.status == "ok"
    assert result.masked_operand_audit.problem_count == 0


def test_masked_operand_audit_accepts_uniformly_shifted_jump_table_targets() -> None:
    mapped = bytearray(b"\x00" * 0x3000)
    struct.pack_into("<II", mapped, 0x2000, 0x401007, 0x40100D)
    ...
    candidate = ObjectFunction(
        name="_foo",
        data=candidate_code + (b"\x00" * 8),
        relocation_offsets=frozenset({3, 21, 25}),
        relocation_references=(
            ObjectRelocationReference(
                offset=3,
                symbol_name="$Ltable",
    ...
    entry = result.masked_operand_audit.entries[0]
    assert entry.target_references[0].jump_table_entries == (7, 13)
    assert entry.candidate_references[0].jump_table_entries == (9, 15)
    assert entry.status == "ok"
    assert result.masked_operand_audit.problem_count == 0
