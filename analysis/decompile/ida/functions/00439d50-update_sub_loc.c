/* database: /Users/banteg/dev/banteg/snail-mail/artifacts/ida/SnailMail_unwrapped.exe.i64 */
/* function: update_sub_loc @ 0x439d50 */
/* selector: update_sub_loc */

// Windows `cRSubLoc::AI()`: updates one runtime `TrackRowCell`/SubLoc. Wall2 tile 14 fires through the owned 20-slot `SubLazerPool` after `first_block_row_count` passes Goldy's z; tiles 22/29/30 and the default path cull against the embedded player's interaction plane, while skirt tiles refresh `runtime_rows[row].attachment_body.color`. The Windows constructor callback and cross-port iOS symbol independently prove the owner.
void __thiscall update_sub_loc(SubLoc *cell)
{
  char *v2; // ecx
  uint8_t tile_id; // al
  double v4; // st7
  float z; // ecx
  uint32_t v6; // edx
  double v7; // st7
  float v8; // edx
  double v9; // st7
  double v10; // st7
  tColour *track_skirt_color; // edi
  Vec3 vector; // [esp+4h] [ebp-34h] BYREF
  float v13[2]; // [esp+10h] [ebp-28h] BYREF
  float v14; // [esp+18h] [ebp-20h]
  Vec3 v15; // [esp+1Ch] [ebp-1Ch]
  tColour v16; // [esp+28h] [ebp-10h] BYREF

  if ( (cell->lane_and_flags & 0x2000) != 0 )
  {
    v2 = (char *)g_game_base;
    if ( !*((_BYTE *)g_game_base + 476705) )
    {
      tile_id = cell->tile_id;
      switch ( tile_id )
      {
        case 0xEu:
          if ( (double)*((int *)g_game_base + 119194) >= *((float *)g_game_base + 1097595) )
            goto LABEL_9;
          if ( random_float_below(100.0) < 4.0 )
          {
            v4 = cell->anchor_position.y + 8.0;
            z = cell->anchor_position.z;
            v13[0] = cell->anchor_position.x;
            v6 = cell->lane_and_flags >> 8;
            v15.y = v4;
            v15.x = v13[0];
            *(_QWORD *)&vector.x = v6 & 0xF;
            v15.z = z;
            v13[1] = v15.y;
            v14 = z;
            v13[0] = (double)*(__int64 *)&vector.x * 0.5 + v13[0];
            v7 = random_signed_float_below(3.0);
            v2 = (char *)g_game_base;
            v8 = *((float *)g_game_base + 1097594);
            v9 = v7 + 8.0 + *((float *)g_game_base + 1097595);
            v16.r = *((float *)g_game_base + 1097593);
            v16.g = v8;
            v15.x = v16.r - v13[0];
            v15.y = v8 - v15.y;
            v10 = v9 - v14;
            v15.z = v10;
            vector = v15;
            if ( v10 >= -4.0 )
              goto LABEL_9;
            normalize_vector(&vector);
            vector.x = vector.x * 0.40000001;
            vector.y = vector.y * 0.40000001;
            vector.z = vector.z * 0.40000001;
            shoot_subgoldy((_DWORD *)g_game_base + 994374, v13, &vector);
          }
          v2 = (char *)g_game_base;
LABEL_9:
          if ( cell->anchor_position.z < (double)*((float *)v2 + 1100223) )
            remove_sub_loc(cell);
          return;
        case 0x16u:
          if ( cell->anchor_position.z >= (double)*((float *)g_game_base + 1100223) )
            return;
          goto LABEL_13;
        case 0x1Du:
        case 0x1Eu:
          if ( cell->attachment_template_record->kind == PATH_TEMPLATE_KIND_WORM )
          {
            cell->render_arg_20 = cell->render_arg_20 - *((float *)g_game_base + 119188) * 0.033333335;
            set_color_alpha(&cell->color, 0.80000001);
            v2 = (char *)g_game_base;
          }
          track_skirt_color = get_track_skirt_color((SubgameRuntime *)(v2 + 476696), &v16);
          *(tColour *)((char *)&unk_6411B8 + (_DWORD)g_game_base + 244 * get_track_cell_row_index(cell)) = *track_skirt_color;
          if ( *((float *)g_game_base + 1100223) - ((double)(int)cell->attachment_template_record->row_span_count + 5.0) > cell->anchor_position.z )
            remove_sub_loc(cell);
          break;
        default:
          if ( cell->anchor_position.z < (double)*((float *)g_game_base + 1100223)
            && (double)(*((_DWORD *)g_game_base + 119196) - 5) > cell->anchor_position.z )
          {
LABEL_13:
            remove_sub_loc(cell);
          }
          break;
      }
    }
  }
}
