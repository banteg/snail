...
// Initializes the authored cRSplash post-run controller, including the Splash backdrop, INTROTEXT.OGG, and the first shell-font message widget before state 30 begins updating it. Android and iOS preserve the same owner and lifecycle as `cRSplash::Init()`.
void __thiscall initialize_thanks_for_playing_screen(cRSplash *splash)
{
  ...
  landscape_script_by_name = load_landscape_script_by_name(&g_game_base->subgame.landscape_manager, aSplashTxt);
  ...
  splash->message_widget = allocate_border(&g_game_base->border_manager);
  v3 = set_color_rgba((tColour *)&color, 1.0, 1.0, 1.0, 1.0);
  initialize_frontend_widget(splash->message_widget, 0x20400002u, aThanksForPlayi, 20, 0.0, 435.0, v3, 2, 0.0);
  splash->message_widget->font_scale = 1.2;
  splash->message_progress = 0.0;
  splash->message_progress_step = 0.0041666669;
  splash->message_state = 0;
}
