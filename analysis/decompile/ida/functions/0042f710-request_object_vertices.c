/* database: /Users/banteg/dev/banteg/snail-mail/artifacts/ida/SnailMail_unwrapped.exe.i64 */
/* function: request_object_vertices @ 0x42f710 */
/* selector: request_object_vertices */

// Ensures one render object has the requested vertex storage; iOS RObject.o names this `cRObject::RequestVertices(int)`.
void __thiscall request_object_vertices(Object *object, int32_t vertex_count)
{
  int32_t v3; // eax

  if ( vertex_count > 0x10000 )
    report_errorf(aTooManyVertice);
  v3 = object->vertex_count;
  if ( v3 > 0 && v3 < vertex_count )
  {
    report_errorf(aReallocationOf);
    free_tracked_memory(object->vertices);
    object->vertex_count = 0;
  }
  if ( !object->vertex_count )
  {
    object->vertex_count = vertex_count;
    object->vertices = (Vec3 *)allocate_tracked_memory(12 * vertex_count, aObjectVertexLi);
    request_object_vertex_colours(object);
  }
}
