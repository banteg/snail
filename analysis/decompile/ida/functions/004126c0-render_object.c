        ...
        char after_sprites)
{
  ...
  uint8_t v8; // cl
  ObjectFlag v9; // eax
  TransformMatrix v10; // [esp+1Ch] [ebp-40h] BYREF
  ...
  if ( (flags & 0x80000) != 0 && (flags & 0x40000) == 0 && object->vertex_count != 0 )
  {
    ...
    v10 = *matrix;
    g_direct3d_renderer.device->vtbl->SetTransform(g_direct3d_renderer.device, 256, &v10);
    ...
    if ( object->texture_group_count > 0 )
    {
      ...
      do
      {
        if ( v8 != 0 )
        {
          ...
          {
LABEL_13:
            if ( object->group_texture_refs[v7] != nullptr )
            {
              if ( (object->flags & 8) != 0 )
              ...
              g_direct3d_renderer.device->vtbl->SetIndices(g_direct3d_renderer.device, object->index_buffer->buffer, 0);
