...
// Exact `cRSaltManager::Add(tVector&)`: allocates one manager-owned Salt from the 40 inline slots, seeds its active state, independent fade/spawn-y/collision-latch fields, and inherited BOD link. The sole Windows caller discards EAX, while the function's exits leave different residual values; analysis conservatively retains int32_t pending stronger ABI evidence and does not claim a slot-index result. Android preserves the vector-only signature; iOS v1.9 adds the owning Goldy argument.
