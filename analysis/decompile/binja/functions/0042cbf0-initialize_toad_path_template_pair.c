...
0042cc16        int32_t lead_count
0042cc16        float start_x
0042cc16        int32_t tail_count
0042cc16        float turn_sign
0042cc16        int32_t esi
0042cc16        if (turn_left == 0)
0042cc3b        lead_count = 2
0042cc43        start_x = 2f
0042cc4b        esi = 2
0042cc4f        turn_sign = 1f
0042cc57        tail_count = 1
0042cc18        esi = 1
0042cc1d        start_x = -2f
0042cc25        turn_sign = -1f
0042cc2d        lead_count = 1
0042cc31        tail_count = 3
0042cc6a        self->width_or_scale = 1f
0042cc71        turn_left.d = tail_count + esi + 0x1a
0042cc75        self->segment_count = tail_count + esi + 0x1a
0042cc7c        self->segment_count_f = fconvert.s(float.t(turn_left.d))
0042cc7f        get_path_nodes(self)
0042cc86        self->has_entry_mesh_transition = 0
0042cc8d        turn_left.d = 0
0042cc91        int32_t lead_count_bound
0042cc91        if (esi s> 0)
0042cc97        int32_t esi_1 = 0
0042cd57        bool cond:2_1
0042cca0        *(&self->primary_samples->center_x + esi_1) = fconvert.s(fconvert.t(start_x))
0042ccaa        *(&self->primary_samples->rotation_scalar_98 + esi_1) = 0
0042ccb4        *(&self->primary_samples->rotation_scalar_94 + esi_1) = 0
0042ccbe        *(&self->primary_samples->special_scalar + esi_1) = 0
0042ccc8        *(&self->primary_samples->lateral_scale + esi_1) = 0x3f800000
0042ccd4        set_matrix_identity(self->primary_samples + esi_1)
0042ccdc        void* eax_4 = self->primary_samples + esi_1
0042ccde        long double x87_r7_2 = float.t(turn_left.d)
0042cce8        *(eax_4 + 0x30) = *(eax_4 + 0x90)
0042ccf2        *(&self->primary_samples->transform.position.y + esi_1) = 0
0042ccf9        *(&self->primary_samples->transform.position.z + esi_1) = fconvert.s(x87_r7_2)
0042cd00        *(&self->primary_samples->delta_length + esi_1) = 0x3f800000
0042cd0c        set_matrix_identity(self->secondary_samples + esi_1)
0042cd22        *(&self->secondary_samples->transform.position + esi_1) = *(&self->primary_samples->center_x + esi_1)
0042cd29        *(&self->secondary_samples->transform.position.y + esi_1) = 0x3efae148
0042cd34        *(&self->secondary_samples->transform.position.z + esi_1) = fconvert.s(fconvert.t(fconvert.s(x87_r7_2)))
0042cd40        *(&self->secondary_samples->delta_length + esi_1) = 0x3f800000
0042cd47        lead_count_bound = lead_count
0042cd4b        esi_1 += 0xa8
0042cd51        cond:2_1 = turn_left.d + 1 s< lead_count_bound
0042cd53        turn_left.d += 1
0042cd57        do while (cond:2_1)
0042cd5d        esi = lead_count_bound
0042cd63        if (tail_count s> 0)
0042cd70        turn_left.d = esi + 0x1a
0042cd85        int32_t esi_5 = (esi + 0x1a) * 0xa8
0042cd95        *(&self->primary_samples->center_x + esi_5) = fconvert.s(fconvert.t(start_x))
0042cd9f        *(&self->primary_samples->rotation_scalar_98 + esi_5) = 0
0042cda9        *(&self->primary_samples->rotation_scalar_94 + esi_5) = 0
0042cdb3        *(&self->primary_samples->special_scalar + esi_5) = 0
0042cdbd        *(&self->primary_samples->lateral_scale + esi_5) = 0x3f800000
0042cdc9        set_matrix_identity(self->primary_samples + esi_5)
0042cdce        struct PathTemplateSample* primary_samples_1 = self->primary_samples
0042cdd1        long double x87_r7_5 = float.t(turn_left.d)
0042cde3        *(&primary_samples_1->transform.position + esi_5) = *(&primary_samples_1->center_x + esi_5)
0042cde9        *(&self->primary_samples->transform.position.y + esi_5) = 0
0042cdf0        *(&self->primary_samples->transform.position.z + esi_5) = fconvert.s(x87_r7_5)
0042cdf7        *(&self->primary_samples->delta_length + esi_5) = 0x3f800000
0042ce03        set_matrix_identity(self->secondary_samples + esi_5)
0042ce19        *(&self->secondary_samples->transform.position + esi_5) = *(&self->primary_samples->center_x + esi_5)
0042ce20        *(&self->secondary_samples->transform.position.y + esi_5) = 0x3efae148
0042ce2f        *(&self->secondary_samples->transform.position.z + esi_5) = fconvert.s(fconvert.t(fconvert.s(x87_r7_5)))
0042ce3a        *(&self->secondary_samples->delta_length + esi_5) = 0x3f800000
0042ce41        esi_5 += 0xa8
0042ce47        int32_t eax_19 = turn_left.d + 1
0042ce48        turn_left.d = eax_19
0042ce4c        lead_count_bound = 0xffffffe6 - lead_count + eax_19
0042ce52        do while (lead_count_bound s< tail_count)
0042ce58        esi = lead_count
0042ce63        turn_left.d = 0
0042ce84        int32_t ebx_3 = esi * 0xa8
0042d02e        bool cond:3_1
0042ce8f        int32_t lead_count_bound_1 = lead_count_bound
0042ceba        float angle = fconvert.s((fconvert.t(1f) - cosine(fconvert.s(float.t(turn_left.d) * fconvert.t(0.241660982f)))) * fconvert.t(0.5f) * fconvert.t(turn_sign) * fconvert.t(1.57079637f))
0042cec2        *(&self->primary_samples->center_x + ebx_3) = fconvert.s(fconvert.t(start_x))
...
0042cf23        *(&self->primary_samples->transform.position + ebx_3) = fconvert.s(st0_2 + st0_2 + fconvert.t(start_x))
0042cf2e        *(&self->primary_samples->transform.position.z + ebx_3) = fconvert.s(float.t(turn_left.d + lead_count))
0042cf35        *(&self->primary_samples->transform.position.y + ebx_3) = 0
0042cf40        int32_t* ecx_27 = &self->primary_samples->transform.basis_up + ebx_3
0042cf48        *ecx_27 = 0
0042cf4e        ecx_27[1] = 0x3f800000
0042cf51        ecx_27[2] = 0
...
0042cfd1        struct PathTemplateSample* primary_sample_bank = self->primary_samples
0042cfdb        long double x87_r7_28 = fconvert.t(*(&primary_sample_bank->transform.basis_up + ebx_3)) * fconvert.t(0.49000001f)
0042cfef        float var_20_1 = fconvert.s(fconvert.t(*(&primary_sample_bank->transform.basis_up.y + ebx_3)) * fconvert.t(0.49000001f))
0042cff6        long double x87_r6_4 = fconvert.t(*(&primary_sample_bank->transform.basis_up.z + ebx_3)) * fconvert.t(0.49000001f)
...
0042d042        int32_t esi_9 = 0
0042d044        struct PathTemplateSample* primary_samples_2 = self->primary_samples
0042d068        float var_2c_2 = fconvert.s(fconvert.t(*(primary_samples_2 + esi_9 + 0xdc)) - fconvert.t(*(&primary_samples_2->transform.position.y + esi_9)))
0042d06f        long double x87_r7_39 = fconvert.t(*(primary_samples_2 + esi_9 + 0xe0)) - fconvert.t(*(&primary_samples_2->transform.position.z + esi_9))
0042d076        *(&primary_samples_2->delta_dir_to_next + esi_9) = fconvert.s(fconvert.t(*(primary_samples_2 + esi_9 + 0xd8)) - fconvert.t(*(&primary_samples_2->transform.position + esi_9)))
0042d078        *(&primary_samples_2->delta_dir_to_next.y + esi_9) = var_2c_2
0042d083        *(&primary_samples_2->delta_dir_to_next.z + esi_9) = fconvert.s(x87_r7_39)
0042d098        *(&self->primary_samples->delta_length + esi_9) = fconvert.s(normalize_vector(&self->primary_samples->delta_dir_to_next + esi_9))
0042d09f        struct PathTemplateSample* secondary_samples = self->secondary_samples
0042d0c3        float var_38_1 = fconvert.s(fconvert.t(*(secondary_samples + esi_9 + 0xdc)) - fconvert.t(*(&secondary_samples->transform.position.y + esi_9)))
0042d0ca        long double x87_r7_46 = fconvert.t(*(secondary_samples + esi_9 + 0xe0)) - fconvert.t(*(&secondary_samples->transform.position.z + esi_9))
0042d0d1        *(&secondary_samples->delta_dir_to_next + esi_9) = fconvert.s(fconvert.t(*(secondary_samples + esi_9 + 0xd8)) - fconvert.t(*(&secondary_samples->transform.position + esi_9)))
0042d0d3        *(&secondary_samples->delta_dir_to_next.y + esi_9) = var_38_1
0042d0de        *(&secondary_samples->delta_dir_to_next.z + esi_9) = fconvert.s(x87_r7_46)
0042d0f3        i += 1
0042d0f4        *(&self->secondary_samples->delta_length + esi_9) = fconvert.s(normalize_vector(&self->secondary_samples->delta_dir_to_next + esi_9))
0042d0fe        esi_9 += 0xa8
...
0042d2e5        vertex = &vertices[(width_cells + 1) * i_1 + esi_10]
...
0042d2f9        ecx_56 = fconvert.s(x87_r6_24)
...
0042d26f        vertex = &vertices[(width_cells + 1) * i_1 + esi_10]
...
0042d27f        ecx_56 = var_28_4
0042d2ff        vertex->z = ecx_56
0042d302        width_cells = self->width_cells
0042d305        esi_10 += 1
0042d306        width_cells_1 = width_cells
0042d30c        turn_left.d = esi_10
0042d310        do while (esi_10 s<= width_cells)
0042d319        i_1 += 1
0042d31a        ebx_4 += 0xa8
...
0042d367        float var_4c = fconvert.s(float.t(eax_68) * fconvert.t(0.125f))
0042d375        float var_54 = fconvert.s(float.t(eax_68 + 1) * fconvert.t(0.125f))
0042d380        int32_t ecx_60 = 0
0042d386        int32_t var_48 = 0
0042d390        turn_left.d = fconvert.s(float.t(turn_left.d) * fconvert.t(0.125f))
0042d39e        float var_50 = fconvert.s(float.t(j + 1) * fconvert.t(0.125f))
0042d3b2        while (true)
0042d3b2        int32_t eax_75 = ecx_60 + ((self->width_cells * i_2 + j) << 1)
0042d3b5        if (ecx_60 != 0)
...
0042d483        ecx_60.w = self->width_cells.w
0042d487        ecx_60.w += 1
0042d490        face_second->vertex_0 = ecx_60.w * i_2.w + j.w + 1
...
0042d4f1        face_second->uv[0].u = var_50
0042d4f4        face_second->uv[0].v = var_4c
0042d4f9        face_second->uv[1].u = edx_42
0042d502        face_second->uv[1].v = var_4c
0042d509        face_second->uv[2].u = edx_42
0042d50c        face_second->uv[2].v = var_54
0042d511        face_second->uv[3].u = var_50
0042d514        face_second->uv[3].v = var_54
0042d3c2        int32_t ecx_62 = eax_75 * 0x30
0042d3c9        struct ObjectFaceQuad* face_first = ecx_62 + facequads
...
0042d3e0        ecx_62.w = self->width_cells.w
0042d3e4        ecx_62.w += 1
0042d3f0        face_first->vertex_1 = ecx_62.w * i_2.w + j.w + 1
