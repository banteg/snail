...
0042d6df        long double x87_r7_8 = float.t(eax_1 + 1)
...
0042d719        int32_t phase_index_1 = 0
0042d71b        *(&self->secondary_samples->transform.position + edi_4) = *(&self->primary_samples->center_x + edi_4)
0042d726        int32_t phase_index = 0
0042d72a        *(&self->secondary_samples->transform.position.y + edi_4) = 0x3efae148
0042d737        *(&self->secondary_samples->transform.position.z + edi_4) = fconvert.s(fconvert.t(fconvert.s(x87_r7_8)))
...
0042d79f        float phase = fconvert.s(float.t(centered) * fconvert.t(6.28318548f) / fconvert.t(fconvert.s(float.t(eax_1))))
...
0042d7cc        phase_index_1 += 1
0042d7db        *(&self->primary_samples->transform.position.y + edi_7) = fconvert.s((fconvert.t(1f) - cosine(phase)) * fconvert.t(0.5f) * fconvert.t(height))
0042d7df        long double x87_r7_18 = float.t(phase_index_1)
...
0042d835        *(&self->secondary_samples->transform.position.y + edi_7) = fconvert.s((fconvert.t(1f) - cosine(phase)) * fconvert.t(0.5f) * fconvert.t(height) + fconvert.t(0.49000001f))
...
0042d87d        *(edi_7 + self->primary_samples - 0xa8) = struct Vec3 {
    .x = 0x3f800000
    .y = 0f
    .z = 0f
}
...
0042d927        *(self->secondary_samples + edi_7 - 0xa8) = struct Vec3 {
    .x = 0x3f800000
    .y = 0f
    .z = 0f
}
...
0042d9bf        edi_7 += 0xa8
0042d9c7        phase_index = phase_index_1
0042d9cb        do while (phase_index_1 s< eax_1)
...
0042daf6        *(&self->primary_samples[self->segment_count] - 0x28) = struct Vec3 {
    .x = 0
    .y = 0f
    .z = 1f
}
...
0042db3d        *(&self->secondary_samples[self->segment_count] - 0x28) = struct Vec3 {
    .x = 0
    .y = 0f
    .z = 1f
}
...
0042db8a        struct Vec3* mesh_vertices = vertices
0042db94        if (self->segment_count s>= 0)
0042db9a        uint32_t width_cells = self->width_cells
0042db9d        int32_t mesh_column_1 = 0
0042dba1        int32_t mesh_column = 0
0042dba5        int32_t mesh_width_cells = width_cells
0042dba9        if (width_cells s>= 0)
0042dbbc        struct PathTemplateSample* primary_samples_1 = self->primary_samples
0042dbc5        long double x87_r7_56 = float.t(centered) - float.t(height) * fconvert.t(0.5f)
...
0042dc2f        vertices = mesh_vertices
...
0042dc7e        vertex = &vertices[(width_cells + 1) * i_1 + mesh_column_1]
...
0042dc07        vertex = &vertices[(width_cells + 1) * i_1 + mesh_column_1]
...
0042dc9f        mesh_column_1 += 1
0042dca0        mesh_width_cells = width_cells
0042dca6        mesh_column = mesh_column_1
0042dcaa        do while (mesh_column_1 s<= width_cells)
0042dcb3        i_1 += 1
0042dcb4        i_2 += 0xa8
...
0042dcd6        int32_t face_column_for_uv = 0
0042dcda        if (self->width_cells s> 0)
0042dce2        int32_t eax_71 = i_2 & 0x80000007
0042dce7        if (eax_71 s< 0)
0042dced        eax_71 = ((eax_71 - 1) | 0xfffffff8) + 1
0042dd01        float v0 = fconvert.s(float.t(eax_71) * fconvert.t(0.125f))
0042dd0f        float v1 = fconvert.s(float.t(eax_71 + 1) * fconvert.t(0.125f))
0042dd1a        int32_t ecx_66 = 0
0042dd20        int32_t var_48_1 = 0
0042dd2a        float u0 = fconvert.s(float.t(centered) * fconvert.t(0.125f))
0042dd38        float u1 = fconvert.s(float.t(j + 1) * fconvert.t(0.125f))
0042dd4c        while (true)
...
0042de8d        face_second->uv[0].u = u1
0042de90        face_second->uv[0].v = v0
0042de95        face_second->uv[1].u = u0
0042de9e        face_second->uv[1].v = v0
0042dea5        face_second->uv[2].u = u0
0042dea8        face_second->uv[2].v = v1
0042dead        face_second->uv[3].u = u1
0042deb0        face_second->uv[3].v = v1
...
0042ddd6        face_first->uv[0].u = u0
0042ddd9        face_first->uv[0].v = v0
0042ddde        face_first->uv[1].u = u1
0042dde7        face_first->uv[1].v = v0
0042ddee        face_first->uv[2].u = u1
0042ddf1        face_first->uv[2].v = v1
0042ddf6        face_first->uv[3].u = u0
0042ddf9        face_first->uv[3].v = v1
