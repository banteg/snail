...
00412495        view_vector.z = fconvert.s(x87_r7_10)
...
00412562        int32_t eax_7 = edge->vertex_a * 3
00412568        struct Vec3* vertices = object->vertices
00412573        struct Vec3* vertex = vertices + (eax_7 << 2)
00412594        struct Vec3 edge_delta
00412594        edge_delta.x = fconvert.s(fconvert.t(view_vector.x) - fconvert.t(*(vertices + (eax_7 << 2))))
00412598        edge_delta.y = fconvert.s(fconvert.t(view_vector.y) - fconvert.t(vertex->y))
004125a4        edge_delta.z = fconvert.s(fconvert.t(view_vector.z) - fconvert.t(vertex->z))
004125ad        float side_b = fconvert.s(dot_vector(&edge_delta, &facequad_normals[edge->normal_b]))
...
0041261c        int32_t eax_16
0041261c        int32_t edx_16
0041261c        edx_16:eax_16 = sx.q(emitted_index_count)
0041262c        int32_t primitive_count = (eax_16 - edx_16) s>> 1
00412637        device_4->vtbl->DrawIndexedPrimitive(device_4, 2, 0, object->grouped_vertex_count, 0, primitive_count)
0041264a        int32_t eax_20 = g_draw_primitive_call_count + 1
0041264b        g_render_triangle_count += primitive_count
00412651        g_draw_primitive_call_count = eax_20
...
004126a1        return device_3->vtbl->SetTransform(device_3, 3, &projection)
