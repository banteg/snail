...
00441ac3        deactivate_sub_lazer_projectile(slot)
0044192c        struct TrackRowCell* eax_7 = get_track_grid_cell_at_world_position(&g_game_base[0x74618], &slot->position)
00441940        char* eax_8 = get_track_runtime_cell_at_world_z(&g_game_base[0x74618], &slot->position)
...
00441ac3        deactivate_sub_lazer_projectile(slot)
00441965        char eax_11
00441965        if ((*eax_8 & 0x40) != 0)
00441977        int32_t eax_10 = *(eax_8 + 0xa4)
...
004419fd        eax_11 = is_point_inside_track_attachment(*(eax_10 + 0x38), fconvert.s(fconvert.t(slot->velocity.x) + fconvert.t(slot->position.x)), fconvert.s(fconvert.t(slot->velocity.y) + fconvert.t(slot->position.y)), fconvert.s(fconvert.t(slot->velocity.z) + fconvert.t(slot->position.z)), eax_10)
00441a04        char* var_2c_4
00441a04        if ((*eax_8 & 0x40) != 0 && eax_11 != 0)
00441a06        var_2c_4 = "lazer path kill\n"
00441ac3        deactivate_sub_lazer_projectile(slot)
00441a13        if ((*eax_8 & 0x80) != 0)
00441a25        int32_t ebx_2 = *(eax_8 + 0xa8)
...
00441aab        char eax_14 = is_point_inside_track_attachment(*(ebx_2 + 0x38), fconvert.s(fconvert.t(slot->velocity.x) + fconvert.t(slot->position.x)), fconvert.s(fconvert.t(slot->velocity.y) + fconvert.t(slot->position.y)), fconvert.s(fconvert.t(slot->velocity.z) + fconvert.t(slot->position.z)), ebx_2)
00441ab2        if (eax_14 != 0)
00441ab4        var_2c_4 = "lazer path2 kill\n"
00441ac3        deactivate_sub_lazer_projectile(slot)
004417f8        if (state == 2)
00441804        uint16_t list_flags = (slot->bod.list_flags).w
00441807        void* ecx_2 = &g_game_base[0x5a8]
...
00441873        *(ecx_2 + 4) = slot->bod.list_next
0044186b        list_prev->list_next = slot->bod.list_next
00441879        slot->bod.list_next = *(ecx_2 + 8)
0044187c        *(ecx_2 + 8) = slot
