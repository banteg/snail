...
00415e3f        int32_t count = registry->count
00415e41        float var_20_1
00415e41        __builtin_strncpy(&var_20_1, "(knN", 8)
00415e4b        if (count s<= 0)
00415eed        return nullptr
00415e57        float* esi_1 = &registry->entries[0].position.z
00415edc        struct ContactTargetEntry (* result)[0x100]
00415e67        long double x87_r7_4 = fconvert.t((esi_1 - 0x10)->entries[0].position.y) - fconvert.t(position->y)
00415e6e        float var_c = fconvert.s(fconvert.t((esi_1 - 0x10)->entries[0].position.x) - fconvert.t(position->x))
00415e7f        float var_8_1 = fconvert.s(x87_r7_4)
00415e83        float var_10_1 = fconvert.s(fconvert.t((esi_1 - 0x10)->entries[0].position.z) - fconvert.t(position->z))
...
00415edc        do while (i s< registry->count)
