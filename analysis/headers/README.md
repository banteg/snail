...
- `completion_types.h`
- `uv run python tools/binja/sync_completion_types.py --target SnailMail_unwrapped.exe.bndb`
- `uv run python tools/ida/sync_completion_screen_types.py`
  - Retires the generic analysis-only `Completion` record after exact layout
    equivalence, verifies the 0x50-byte `cRCompletion` tail at
    `cRSubGame +0x12727d8` between `EnemyManager` and `cRTimesUp`, then
    replays the exact `UnInit`, `Init`, `AI`, and `RegisterParcel` ABIs.
...
Replay `update_golb_ai`'s kind-zero Sprite, byte-indexed Slug cursor, direct
and splash garbage chains, and wall-impact vector lifetimes with `uv run python
tools/binja/sync_golb_ai_lifetimes.py --target
SnailMail_unwrapped.exe.bndb`.

Replay `create_golb`'s hotspot/velocity cursors, kind-zero Sprite owner,
vapour sentinel, subgame-rate pointer split, and contact-target result with
...
`0x14`-byte gap, `cRTipManager` at `+0x12e6f58`, and the final four-byte gap into
