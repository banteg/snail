/* Binary Ninja import wrapper for the official BASS 2.0 C/C++ header file.
   Differences from `bass.h`:
   - uses `"wtypes.h"` so BN resolves the local shim
   - uses `unsigned long long` instead of `unsigned __int64` for portability
*/

/* BASS 2.0 C/C++ header file, copyright (c) 1999-2003 Ian Luck.
...
#define BASS_CONFIG_PAUSE_STOPOUT	13
...
BOOL BASSDEF(BASS_MusicSetVolume)(HMUSIC handle, DWORD chanins, DWORD volume);
DWORD BASSDEF(BASS_MusicGetVolume)(HMUSIC handle, DWORD chanins);
...
HCHANNEL BASSDEF(BASS_SamplePlay3D)(HSAMPLE handle, const BASS_3DVECTOR *pos, const BASS_3DVECTOR *orient, const BASS_3DVECTOR *vel);
HCHANNEL BASSDEF(BASS_SamplePlay3DEx)(HSAMPLE handle, const BASS_3DVECTOR *pos, const BASS_3DVECTOR *orient, const BASS_3DVECTOR *vel, DWORD start, int freq, int volume, BOOL loop);
...
HSTREAM BASSDEF(BASS_StreamCreateFileUser)(BOOL buffered, DWORD flags, void *proc, DWORD user);
...
HRECORD BASSDEF(BASS_RecordStart)(DWORD freq, DWORD flags, RECORDPROC *proc, DWORD user);
