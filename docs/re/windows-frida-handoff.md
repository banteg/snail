...
- `0x420cb0` `update_track_attachment_follow_state`
- `0x43af60` `end_track_attachment_follow_state`
- `0x42b920` `compute_kind42_attachment_transform` (`nonlinear_42` family in Zig)
...
- `movement_flags_update` now samples the
  `Player::shooting_tier -> Player::shoot_flags` table directly at `0x43a1a0`;
  its older event and payload names remain stable for trace compatibility
...
- the current checked-in default is now the stable `attachment_survey` profile:
  ...
  - `salt_deactivate`
  - `slug_spawn`
- that profile is intended for broader attachment-family survey work, including Challenge or random-level runs, while still leaving death/completion and the known crashing mid-function attachment probes off
...
- `cRSubGame::runtime_flags` at `+0x4c` now has sixteen statically proved
