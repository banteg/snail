...
- `+0x2730`: `shoot_cooldown_progress`
- `+0x2734`: `shoot_cooldown_step`
  - selected by authored `cRSubGoldy::SetShootFlags()` for each weapon family
    and consumed only by the `cRSubGoldy::AI()` firing gate
  - existing Frida captures retain `movement_fire_progress` and
    `movement_fire_progress_step` as compatibility keys
...
- `player + 0x2984` is the inline exact 0x19b4-byte `Snail` (`cRSnail`)
  ...
  - `+0x1894`: `invincible_shell`
    - exact 0x98-byte authored `cRInvincible` visual owner
    - `+0x28`: shell color
...
- `player + 0x4344` is the real inline 0x18-byte authored `cRSquidge` consumed by `update_subgoldy` and copied into `player->presentation.visual_root`
  ...
  - the paired narrow replay retires the generic analysis-only `Squidge`, verifies the exact six-float owner and embed, and applies all four authored receiver ABIs to both analyzers
...
- `player + 0x350..+0x370` is now a typed camera-presentation slice
  ...
  - Windows initializes the `+0x14` lane, and Android independently clears its
    homologous second-envelope amplitude slot, as a float-shaped zero between
