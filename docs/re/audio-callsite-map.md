...
| Audio | Native caller | Current interpretation | Current port equivalent | Gap |
| --- | --- | --- | --- | --- |
| `sfx 49` | [`update_row_event_display`](../../artifacts/ida/functions/00404cf0-update_row_event_display.c) at `0x404e6f` | `SUMMARY_PENDING` final-bonus payout cue for the zero-target path | `Completion` / `cRCompletion` | partial; the normal final-parcel payout is ported through `register_parcel_delivery`, while the zero-target summary path still needs a concrete runtime case |
| `sfx 8` | [`update_row_event_display`](../../artifacts/ida/functions/00404cf0-update_row_event_display.c) at `0x404ed2` | row-event accept cue when the controller advances to `CONTINUE_ACCEPTED` | `Completion` UI staging | ported; the cue now keys from the recovered state transition instead of the `+0x18` gate byte |
...
| Audio | Native caller | Current interpretation | Current port equivalent | Gap |
| --- | --- | --- | --- | --- |
| `sfx 17..24` | [`play_subgoldy_shoot_sfx`](../../analysis/decompile/ida/functions/0043afd0-play_subgoldy_shoot_sfx.c) at `0x43b114` | dedicated shoot-state family chosen from `Player::shoot_flags`, with distance attenuation during attachment exit | player-fire families now use native random family selection, the recovered tier-to-family ladder, tier-owned fire cooldown steps, the startup cooldown reseed window from `update_subgoldy`, explicit live press-vs-held input lanes, the runtime fire feature flag, replay raw-bit fire gating, native attachment-exit suppression/gain, and the recovered turbo-spread plus laser/rocket speed ratios from `create_golb` | partial only at the port boundary: the Windows source is closed as mouse-left → controller slot 0 → root-owned `InputState` press/held lanes, and the emitter is exact `cRSubGoldy::Shoot`; Zig still flattens that device/controller ownership |
| `sfx 15` | [`shoot_sub_lazer_pool`](../../analysis/decompile/ida/functions/00441ad0-shoot_sub_lazer_pool.c) at `0x441b15` | runtime `cRSubLazerManager` Wall2 shot cue after a free pool slot is spawned | SubLazer successful-allocation generation and original spawn position in `NativeGameplaySoundCues`, dispatched as `SFX2/ENEMYFIRE.OGG` with the recovered 25-unit distance attenuation from [`play_sound_effect_at_position`](../../analysis/decompile/ida/functions/0044dce0-play_sound_effect_at_position.c) | ported for the pool-allocation edge and distance gain; native left/right pan is still not represented by the Raylib playback wrapper |
| `sfx 41` | [`update_subgoldy`](../../artifacts/ida/functions/0043b120-update_subgoldy.c) at `0x43c3f8` | trampoline landing cue on runtime tile `22` | runner trampoline handling in `gameplay.zig` | ported for the bounce edge; the port's `SquidgeState` now seeds the recovered native `cRSquidge` Y oscillator |
| `sfx 47` | [`update_subgoldy`](../../artifacts/ida/functions/0043b120-update_subgoldy.c) at `0x43c4c1` | first-entry cue for runtime tile `14` (`'|'`) before the native hold-and-drop timer runs | runner wall or barrier contact in `gameplay.zig` | ported; the port now keeps the one-shot cue and mirrors the tile-`0x0e` z snap, Z squidge seed, and one-second hold/drop controller |
...
| Audio | Native caller | Current interpretation | Current port equivalent | Gap |
| --- | --- | --- | --- | --- |
| `sfx 39/40` | [`handle_subgoldy_collisions`](../../artifacts/ida/functions/00444cf0-handle_subgoldy_collisions.c) at `0x44500a` | garbage collision impact family after score add and `+0.04` damage | garbage-hit handling and asteroid-impact pair in `playGameplayRunnerAudio()` | ported; runner queues the native RNG-selected impact variant on garbage collision only, and salt / defeated-slug events no longer borrow this family |
| `sfx 14` | [`handle_subgoldy_collisions`](../../artifacts/ida/functions/00444cf0-handle_subgoldy_collisions.c) at `0x44542f` | health pickup cue | live health-pickup runtime slots with the recovered `world_y >= 0.49`, `delta_z < 1.0`, `abs(delta_y) < 0.4`, and normalized-distance `< 0.98` gates, dispatched as the shipped `heart` sound plus an 8-smoke pickup burst using the native `Player.velocity` carry lanes | ported for the recovered owner boundary: the inline `SubHealth` actor owns the pickup BOD and borrows one pickup sprite, while the burst is eight ordinary SpriteManager records; no dedicated particle BOD exists |
| `sfx 43` | [`handle_subgoldy_collisions`](../../artifacts/ida/functions/00444cf0-handle_subgoldy_collisions.c) at `0x4456f2` | ring types `3/7` negative-velocity cue | live runtime ring-slot collision handling keyed from the exact native ring-kind `3/7` token, dispatched as the shipped slow-ring cue plus the recovered `velocity.z = -0.1` shove and short zero-crossing recovery | partial; the negative shove and the matching `track_center_x * 0.5` forward impulse for non-slow live ring effects now share the runner velocity lane, but the broader native forward-speed owner is still not fully literal |
| `sfx 1..7` | [`handle_subgoldy_collisions`](../../artifacts/ida/functions/00444cf0-handle_subgoldy_collisions.c) at `0x445763` | tiered weapon-upgrade pickup family after ring types `4/5` and `8`, indexed by the post-increment `shooting_tier` | live runtime ring-slot collision handling plus the shipped `PW1..PW7` pickup ladder keyed from the exact native ring-kind token and shooting tier | partial; the ring-slot owner and pickup ladder are now ported, but the deeper shoot-flag presentation owner is still flatter in Zig |
...
3. Recover the native damage-warning owner instead of only mirroring its visible fill and warning loop.
   ...
   Current Zig status: the runner now owns the warning actor cadence, display-fill smoothing, `sfx 50` loop edge, and explicit context gates for native warning startup, accelerated drain, and state-2 exits. The Windows `stop_warning_sample` contract is closed as an immediate start-then-stop of the returned channel with no stored handle, and the completion-handoff drain gate is closed. The remaining open part is the hit-flash/global-gate exit and its presentation timing.
   Next trace boundary: `apply_damage_gauge_delta`, `update_damage_gauge`.
...
5. Recover the movement-state sound controller instead of treating gameplay audio as mostly event-based.
   ...
   Native implication: there is a dedicated shooting controller that samples
   `shoot_flags`, picks families, and even switches to an attenuated path during
   attachment exit.
   Current Zig gap: the port has the recovered family ladder, selector-owned cooldown steps, startup cooldown reseed, live press-vs-held lanes, replay raw-bit gate, runtime fire feature flag, attachment-exit suppression/gain, and turbo-spread plus laser/rocket ratios. The native device chain is now known as mouse-left → controller slot 0 → root-owned `InputState`, but Zig still flattens that ownership boundary.
   Next trace boundary: `play_subgoldy_shoot_sfx`,
   `set_subgoldy_shoot_flags`, and the fire-gate block in `update_subgoldy`.
...
6. Split collision and powerup feedback back into their native families.
   ...
   Current Zig suspicion: the exact ring-kind ladder is now ported for `sfx 1`, `sfx 1..7`, `sfx 42`, `sfx 43`, and `voice 5`, and health collision plus pickup particles now use the recovered runtime slot gates and `Player.velocity` carry lanes. Garbage impact audio now matches the native `sfx 39/40` owner, and weapon-change audio keys from recovered visual channel deltas; parcel, jetpack, and the deeper jetpack presentation owner are still not fully literal.
