...
| Audio | Native caller | Current interpretation | Current port equivalent | Gap |
| --- | --- | --- | --- | --- |
| `sfx 17..24` | [`play_movement_state_sound`](../../artifacts/ida/functions/0043afd0-play_movement_state_sound.c) at `0x43b114` | dedicated movement-state family chosen from `movement_flags`, with distance attenuation during attachment exit | player-fire families now use native random family selection, the recovered selector-to-family ladder, selector-owned fire cooldown steps, explicit live press-vs-held input lanes, the runtime fire feature flag, replay raw-bit fire gating, and native attachment-exit suppression/gain | partial; the remaining gap is the deeper emitter owner and literal input-device source, not the family choice, replay gate, cadence, or exit attenuation |
...
| `sfx 43` | [`handle_subgoldy_collisions`](../../artifacts/ida/functions/00444cf0-handle_subgoldy_collisions.c) at `0x4456f2` | ring types `3/7` negative-velocity cue | exact native ring-kind `3/7` token in runner collision handling, dispatched as the shipped slow-ring cue | partial; the exact negative-velocity motion lane is still owned by the runner, but the audio family is now keyed from the native ring-kind path |
| `sfx 1..7` | [`handle_subgoldy_collisions`](../../artifacts/ida/functions/00444cf0-handle_subgoldy_collisions.c) at `0x445763` | tiered weapon-upgrade pickup family after ring types `4/5` and `8`, indexed by the post-increment movement selector | shipped `PW1..PW7` pickup ladder keyed from the exact native ring-kind token plus `movement_flag_selector`, with the shared `weapon_change` cue still following weapon presentation | partial; the pickup family now follows the native ring-kind ladder, but the deeper movement-flag presentation owner is still flatter in Zig |
| `sfx 1` | [`handle_subgoldy_collisions`](../../artifacts/ida/functions/00444cf0-handle_subgoldy_collisions.c) at `0x44578d` | ring type `1` pickup cue | exact native ring-kind `1` token in runner collision handling | ported; the app now keys `PW1` from the recovered ring-kind `1` path instead of generic pickup progression |
| `sfx 42` | [`handle_subgoldy_collisions`](../../artifacts/ida/functions/00444cf0-handle_subgoldy_collisions.c) at `0x44580d` | explode-ring pickup, immediately before `initialize_nuke` | exact native ring-kind `2/6` token in runner collision handling plus existing explode-ring gameplay/runtime | partial; the pickup cue is now keyed from the native ring-kind path, but the underlying `initialize_nuke` owner is still not a literal port |
...
5. Recover the movement-state sound controller instead of treating gameplay audio as mostly event-based.
   ...
   Current Zig suspicion: the port now has the recovered family ladder, selector-owned cooldown steps, explicit live press-vs-held input lanes, replay raw-bit gating, the runtime fire feature flag, and attachment-exit suppression/gain, but it still does not mirror the deeper emitter owner or literal input-device source.
