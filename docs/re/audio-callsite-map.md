...
| `sfx 41` | [`update_subgoldy`](../../artifacts/ida/functions/0043b120-update_subgoldy.c) at `0x43c3f8` | trampoline landing cue on runtime tile `22` | runner trampoline handling in `gameplay.zig` | ported for the bounce edge; the remaining gap is only the accompanying squidge burst |
| `sfx 47` | [`update_subgoldy`](../../artifacts/ida/functions/0043b120-update_subgoldy.c) at `0x43c4c1` | first-entry cue for runtime tile `14` (`'|'`) before the native hold-and-drop timer runs | runner wall or barrier contact in `gameplay.zig` | ported; the port now keeps the one-shot cue and mirrors the tile-`0x0e` z snap plus one-second hold/drop controller |
