...
| `sfx 43` | [`handle_subgoldy_collisions`](../../artifacts/ida/functions/00444cf0-handle_subgoldy_collisions.c) at `0x4456f2` | ring types `3/7` negative-velocity cue | live runtime ring-slot collision handling keyed from the exact native ring-kind `3/7` token, dispatched as the shipped slow-ring cue plus the recovered `velocity.z = -0.1` shove and short zero-crossing recovery | partial; the negative-velocity shove itself is now ported through a runner-local override, but the broader native forward-speed controller behind that same motion lane is still scaffolding |
