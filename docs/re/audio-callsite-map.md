...
| Audio | Native caller | Current interpretation | Current port equivalent | Gap |
| --- | --- | --- | --- | --- |
| `sfx 49` | [`update_row_event_display`](../../artifacts/ida/functions/00404cf0-update_row_event_display.c) at `0x404e6f` | row-event state-`3` final-bonus payout cue for the zero-target path | `RowEventDisplayController` | partial; the normal final-parcel payout is ported through `register_parcel_delivery`, while the zero-target state-`3` payout path still needs a concrete runtime case |
| `sfx 8` | [`update_row_event_display`](../../artifacts/ida/functions/00404cf0-update_row_event_display.c) at `0x404ed2` | row-event accept or continue cue when the controller advances to state `5` | `RowEventDisplayController` UI staging | ported; the cue now keys from the controller's transition into state `5` instead of the `+0x18` gate byte |
...
| `voice 4` `mode 1` | [`update_track_attachment_follow_state`](../../artifacts/ida/functions/00420cb0-update_track_attachment_follow_state.c) at `0x420d30` | static callsite guarded by `sample_index + 1 == template->sample_count << 1` | attachment follow update in `gameplay.zig` | unresolved; raw BN plus IDA also show begin seeding `sample_index = 0` and the same helper terminating at `sample_index == template->sample_count`, so this branch is contradictory under the current typed counters and should not be ported yet |
