...
| Audio | Native caller | Current interpretation | Current port equivalent | Gap |
| --- | --- | --- | --- | --- |
| `sfx 17..24` | [`play_movement_state_sound`](../../artifacts/ida/functions/0043afd0-play_movement_state_sound.c) at `0x43b114` | dedicated movement-state family chosen from `movement_flags`, with distance attenuation during attachment exit | player-fire families in `playGameplayRunnerAudio()` now use native random family selection, the recovered selector-to-family ladder, and the native attachment-exit distance gain | partial; the remaining gap is the native movement-state owner and gate, not the per-family sample choice, family ladder, or exit attenuation |
...
| `sfx 1..7` | [`handle_subgoldy_collisions`](../../artifacts/ida/functions/00444cf0-handle_subgoldy_collisions.c) at `0x445763` | tiered weapon-upgrade pickup family after ring types `4/5` | shipped `PW1..PW7` pickup ladder plus the existing `weapon_change` cue | partial; the port now uses the native pickup-family assets, but it still keys them from generic powerup progression instead of the native ring-kind ladder |
