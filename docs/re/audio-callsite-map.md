...
5. Recover the movement-state sound controller instead of treating gameplay audio as mostly event-based.
   ...
   attachment exit.
   Current Zig suspicion: the port now has the recovered family ladder, selector-owned cooldown steps, the startup cooldown reseed window, explicit live press-vs-held input lanes, replay raw-bit gating, the runtime fire feature flag, attachment-exit suppression/gain, and the recovered turbo-spread plus laser/rocket speed ratios, but it still does not mirror the literal input-device source.
