...
6. Split collision and powerup feedback back into their native families.
   ...
   Current Zig suspicion: the exact ring-kind ladder is now ported for `sfx 1`, `sfx 1..7`, `sfx 42`, `sfx 43`, and `voice 5`, and health collision plus pickup particles now use the recovered runtime slot gates and `Player.velocity` carry lanes. Parcel, jetpack, garbage-impact, and the deeper weapon/jetpack presentation owners are still not fully literal.
