...
14. Phase B2 — **partial**. `app/level_loader.zig` owns level/segment loading, active segment sync, row-message prompt dispatch, run-tuning helpers, track-build seeds, and mode-to-runtime helpers. `app/run_result.zig` owns run-result data plus summary/ranking/completion-flow helpers, high-score persistence commits, and completion-screen reveal state. Remaining B2 work: completion-screen result transitions and outer bridge.
