...
15. Phase B3 — **partial**. `app/frontend_flow.zig` owns frontend phase entry, start-phase setup, menu action dispatch, high-score screen entry, exit prompt routing, frontend level launch, selected-record launch, route-map entry, and route-map action/route stepping helpers. `app/frontend_input.zig` owns hover/activation queue state, widget shortcut dispatch, frontend keyboard navigation/value stepping, and widget animation/snap state. `app/route_map_state.zig` owns route-map card open/close and highlight state. Remaining B3 work: mouse hit-testing.
