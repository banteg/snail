...
| `app/render_phase.zig` | Shared render phase policy: gameplay-backdrop detection, light-streak visibility, and live subgame camera handoff for background streaks. |
...
| `app/screenshots.zig` | Screenshot request + capture path: `Request`, auto-screenshot queueing, and window capture/export. |
...
8. Phase A7b — **done**. `gameplay/phase.zig` owns composed `CutsceneState` and `HandoffState`; `Runner` now exposes those lanes as `runner.cutscene` and `runner.handoff`, and camera/audio/tests reach through the grouped state instead of flat fields.
...
12. Phase A9b — partially done. `gameplay/motion.zig` owns `ReplayState` for replay world/lane/flag/fade latches. Remaining motion state: row/lane advance, current sample/runtime tile hint, traversable bounds, and previous row/lane snapshots.
