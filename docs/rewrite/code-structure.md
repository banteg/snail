...
| Target file | Content |
| --- | --- |
| `gameplay/runner_state.zig` | Pure data types: `RunnerInput`, `ReplayDirective`, `AttachmentHint`, `MovementMode`, `SessionMode`, `DeathCause`, `RecentEvent`, `EncounterCounters`, `Stopwatch`, `DamageWarningState`, `DamageWarningActorState`, `DamageGaugeRuntime`, `DamageWarningActor`, `SnailSkinSlot`, `SnailSkinTransition`, `JetpackWarningBand`, `JetpackGauge`, `WeaponChannelStates`, `Projectile`, `AttachmentFollowState`, `AttachmentExitCarryover`, `InstalledAttachmentEntry`, `LaunchState`, `WorldFrame`, `AttachmentCameraProgress`, `FallState`, `RunnerPhase`, `RunnerHandoff`. Plus the constants that live with these types (cutscene ids, fall thresholds). |
