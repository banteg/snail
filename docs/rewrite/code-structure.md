...
| `app/run_result.zig` | Pending run result data and pure helpers: `Result`, `Persistence`, `Outcome`, `CompletionFlowOwner`, `RespawnBridgeState`, `StandalonePostLevelHighScoreEntry`, summary conversion, completion-owner return targets, postal unlock/return helpers, and high-score rank preview. Remaining accounting methods still delegate from `main.zig`. |
