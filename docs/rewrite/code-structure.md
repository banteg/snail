...
- Archive member bytes and catalogs are already resident for the full app lifetime, so small global resource sets should follow that shape too. Gameplay sound effects are app-global boot resources owned by `AppState`, not per-level state; future resource-store work should prefer cached uploaded handles over reload-on-navigation where the shipped asset set is small.
