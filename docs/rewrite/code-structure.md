...
15. Phase B3 — **partial**. `app/frontend_flow.zig` owns frontend phase entry, start-phase setup, menu action dispatch, high-score screen entry, exit prompt routing, frontend level launch, selected-record launch, and route-map entry. Remaining B3 work: move mouse/keyboard frontend activation and route-map card state helpers once the UI-input boundary is clearer.
