...
| `app/frontend_input.zig` | Frontend activation state: hover target updates, queued activation dispatch, pending fade/activation handoff, widget shortcut code mapping, and shortcut activation handling. |
...
15. Phase B3 — **partial**. `app/frontend_flow.zig` owns frontend phase entry, start-phase setup, menu action dispatch, high-score screen entry, exit prompt routing, frontend level launch, selected-record launch, and route-map entry. `app/frontend_input.zig` owns hover/activation queue state and widget shortcut dispatch. Remaining B3 work: mouse hit-testing, keyboard selection stepping, widget animation snapshots, and route-map card state helpers.
