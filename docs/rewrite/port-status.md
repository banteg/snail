...
- active segment `Sample=` / `Message=` routing in the default level path: `partial`
  ...
  - note: the port now uses authored `Duration:` values when present, shows tutorial guidance as a single top-centered timed banner instead of the old footer/debug shell or bottom speaker card, and clears the previous tutorial tip before showing the next one. `Sample=` playback on `VOICE/TUT*.OGG` also now runs on a dedicated voice channel instead of the old shared preview-sound slot. It still does not port the original row-event actor/controller, score-flush behavior, or the exact tutorial voice/tip ownership beyond the current single-owner approximation.
