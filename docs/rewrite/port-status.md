...
- active segment `Sample=` / `Message=` routing in the default level path: `partial`
  ...
  - note: the port now uses authored `Duration:` values when present, clears the previous tutorial tip before showing the next one, and treats the common tutorial row-event guidance as a single-owner `cRTip`-style card with an `OK` button instead of the older debug footer/banner scaffolding or custom dialogue box. The tutorial startup prompt also now uses the recovered centered text-only shell-font widget path instead of custom HUD text. `Sample=` playback on `VOICE/TUT*.OGG` runs on a dedicated voice channel instead of the old shared preview-sound slot, and tips no longer tick down while hidden under pause/overlay, which is closer to the original hide/unhide behavior. It still does not port the original row-event actor/controller, score-flush behavior, or the full three-slot general-purpose tip manager outside the tutorial flow.
