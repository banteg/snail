...
- gameplay subgame lifecycle: `partial`
  ...
  - note: the port now carries a second Goldy-side controller for completion and death handoffs, with Windows-backed respawn rules for postal and tutorial plus final-loss rules for challenge and time-trial. The runner also now keeps a dedicated stopwatch shaped after `advance_timer_counters` instead of deriving route time only from `tick_count` at the result screen. Postal and tutorial respawns no longer happen as a hidden runner-local reset; they now surface as an explicit gameplay handoff and are rebuilt by the app-level level controller while preserving total score, timer state, and remaining visible lives. That is directionally closer to the original `Goldy -> state 28 -> outer subgame rebuild` split, though still simpler than the original reload path. Attachment-follow is still missing the full Windows installed bank installer and swept local-frame entry test, but it is no longer keyed only from raw authored row tags: the preview now derives a first installed attachment row map from the runtime attachment tiles, and built templates drive live progress, world pose, camera forward/up, kind-42 surface tilt, natural-end exit pose, a first width-based side-exit rule, and a dedicated `SUPERTRAMP` launch exit instead of only row-midpoint hints. The live gameplay camera now also stays on the built attachment/launch frame through those exits instead of dropping straight back to the flat chase view. The gameplay viewport also now reuses the textured track scene for supported `Track:` sets instead of always falling back to the raw debug preview, so those built attachment families are visible in actual play as well as in the segment browser. It also routes conservative floor-gap proxies straight into death resolution instead of the scripted hit-side cutscene: a lane with no sampled floor inside an otherwise grounded row, outside attachment-follow, without an active jetpack, and without an authored `NoFall` annotation.
