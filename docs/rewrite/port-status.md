...
- gameplay Turbo model render: `partial`
  ...
  - note: the gameplay viewport now draws a live Turbo model instead of only the track scene. The first pass attaches one `TURBO-BOBALONG` animation family to the runner's current world frame, including attachment-follow/launch orientation, and the tutorial weapon tiers now switch visible mounts between a top blaster, side lasers, and a rocket launcher instead of one generic weapon presentation. The anchor, facing, and state-specific animation selection are still rough.
