...
- tutorial gameplay actor presentation: `partial`
  ...
  - note: the live gameplay viewport now renders slug, garbage, salt, health, jetpack, parcel, and ring/powerup actors as visible gameplay-facing sprites/billboards instead of leaving tutorial interactions as invisible logic or debug markers. This is still a tutorial-first presentation pass: some actors are still billboards instead of the original runtime-object/controller path, and the exact owner/update semantics are not yet fully ported.
  - replace when: the original runtime actor ownership, animation/state switching, and any non-billboarded presentation paths are ported for these hazards/pickups
