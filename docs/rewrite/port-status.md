...
- damage-gauge hazard and recovery deltas: `partial`
  ...
  - note: the runtime now enters a partial warning/fill/drain state when the gauge reaches `1.0`, matching the recovered `cRDamageGuage::AI()` direction that full damage is not an immediate death path. Until the Windows hazard-pool split is ported, the current runner treats the shipped slug collision as the conservative lethal scripted hazard that enters the death controller.
