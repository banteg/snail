...
- damage-gauge hazard and recovery deltas: `partial`
  ...
  - note: Windows now splits contact damage from jetpack countdown. The current port's `damage_gauge` aligns with the collision-side controller at player `+0x3c4`, not the separate jetpack gauge at `+0x2750`. It also now treats the first slug hit as the scripted death-cutscene handoff instead of an immediate `+1.0` gauge fill, which is closer to `handle_subgoldy_collisions`. The gameplay HUD carries a dedicated gauge widget plus the postal-only 9-slot life strip that Windows toggles through `show_subgoldy_lives`, and the runner now also routes obvious side-gap floor losses into the same death-cutscene path. The original hazard-pool suppression flags, repeated-slug-contact damage, warning actor behavior, exact sprite-driven presentation, and the unresolved `+0.02` collision path are still not ported.
  - replace when: the unresolved `+0.02` collision path, the `bool`-gated suppression semantics, warning actor behavior, attachment-carried void rows, exact fall-state timing, and the original gauge presentation are ported
