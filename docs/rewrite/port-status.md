...
- level completion and post-level score flow: `partial`
  ...
  - note: the runtime now has a distinct post-finish cutscene phase before the completion screen, and `Continue` can hand off into the shared high-score screen for inline name entry instead of a separate post-level overlay, but the cutscene camera blend is still a conservative approximation of `cRCutScene::AI()`. Windows also shows that Goldy carries a separate cutscene-controller state polled from `update_subgoldy`, with both completion and some hazard-death paths entering scripted camera states before the final frontend handoff. Postal scoring keeps the recovered final-bonus path separate from challenge runs and applies that bonus on the last parcel event instead of waiting until the run ends, but both modes still omit the missing slug/jetpack/replay-side score events.
