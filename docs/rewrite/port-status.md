...
- gameplay subgame lifecycle: `partial`
  - evidence: Binary Ninja decompile of `initialize_subgame`, `build_subgame_level`, `complete_subgame`, `destroy_subgame`, `update_subgame`, `initialize_subgoldy`, `show_subgoldy_lives`, `update_subgoldy`, `initialize_subgoldy_fall_state`, `initialize_subgoldy_death`, `update_subgoldy_resurrect`, `handle_subgoldy_collisions`, `initialize_damage_gauge`, `update_damage_gauge`, `advance_timer_counters`, `update_subgame_camera`, `initialize_cameraman`, and `update_cameraman`, plus cross-port Android and iOS symbols for `cRSubGame::{Init, BuildLevel, Complete, AI, CameraAI, UnInit}`, `cRSubGoldy::{Init, ShowLives, DeathInit, RessurectAI, Collision, AI}`, `cRDamageGuage::{Init, AI}`, and `cRCameraman::{Init, AI}`
...
- damage-gauge hazard and recovery deltas: `partial`
  ...
  - note: the runtime now enters a partial warning/fill/drain state when the gauge reaches `1.0`, matching the recovered `cRDamageGuage::AI()` direction that full damage is not an immediate death path. The gameplay HUD also now carries a dedicated gauge widget plus the postal-only 9-slot life strip that Windows toggles through `show_subgoldy_lives`, but the original hazard-pool suppression flags, warning actor behavior, and exact sprite-driven presentation are still not ported.
