...
- active segment `Sample=` / `Message=` routing in the default level path: `partial`
  ...
  - note: the port now uses authored `Duration:` values when present, shows tutorial guidance as a single top-centered timed banner instead of the old footer/debug shell or bottom speaker card, and clears the previous tutorial tip before showing the next one. `Sample=` playback on `VOICE/TUT*.OGG` also now runs on a dedicated voice channel instead of the old shared preview-sound slot. Tips no longer tick down while hidden under pause/overlay, which is closer to the original hide/unhide behavior. It still does not port the original row-event actor/controller, score-flush behavior, or the exact tutorial voice/tip ownership beyond the current single-owner approximation.
...
- tutorial gameplay actor presentation: `partial`
  ...
  - note: the live gameplay viewport now renders slug, garbage, salt, health, jetpack, parcel, ring/powerup, and `=` turret actors instead of leaving tutorial interactions as invisible logic or debug markers. Salt now uses the shipped `X/SALT.X2` model path rather than a font/icon stand-in, while slug and garbage remain tutorial-first sprite actors. Turbo also now shows tiered gameplay weapons and projectiles instead of one generic blaster presentation: top blaster with turbo shots, side lasers with dual laser shots, and a rocket launcher with rocket shots, plus the invincible shell overlay. Tutorial/combat SFX are also now wired into live gameplay for fire, pickups, invincible/slow powerups, explosive rings, and basic impact feedback on separate preloaded effect sounds instead of relying only on segment `Sample=` playback. Weapon fire now uses the recovered multi-sound families (`TurboFire1/2`, `Laser1/2/3`, `Rocket1/2/3`) instead of one fixed shot sound per tier, and asteroid-style impacts now randomize across the shipped impact pair. Garbage collisions now shave forward speed, kick Turbo sideways, and emit a first smoke aftermath layer, which is closer to the original “Asteroids slow me down” path. Explode rings now clear nearby garbage and defeat nearby slugs, salt blocks shots instead of being consumed, and `=` rows currently behave as lethal turret obstacles rather than staying invisible.
