...
- damage-gauge hazard and recovery deltas: `partial`
  ...
  - note: Windows now splits contact damage from jetpack countdown. The current port's `damage_gauge` aligns with the collision-side controller at player `+0x3c4`, not the separate jetpack gauge at `+0x2750`. The gameplay HUD carries a dedicated gauge widget plus the postal-only 9-slot life strip that Windows toggles through `show_subgoldy_lives`, but the original hazard-pool suppression flags, warning actor behavior, exact sprite-driven presentation, and the unresolved `+0.02` collision path are still not ported.
