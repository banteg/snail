...
- level completion and post-level score flow: `partial`
  ...
  - note: postal and challenge scores now accumulate inside the runner using the recovered `ScoreAdd` constants for rings, health, parcel pickup/register, and the completion bonus, but still omit the missing garbage/slug/jetpack/replay-side score events
