...
- active segment `Sample=` / `Message=` routing in the default level path: `partial`
  ...
  - note: the port now queues up to three transient segment prompts and uses authored `Duration:` values when present instead of pinning the current `Message=` text in the footer for the whole segment. Normal gameplay also no longer renders the old debug footer shell for these messages; tutorial guidance now shows through compact stacked gameplay tip cards instead of the earlier invented footer speaker panel. It still does not port the original row-event actor/controller, score-flush behavior, or voice mixing.
