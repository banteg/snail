...
- exact challenge, time-trial, and replay-sensitive return routing beyond the currently recovered selected-level-record final-loss `0x1a` lane
- the successful selected-level-record post-run branch that uses state `0x1b`, plus the meaning of the remaining app-side gate at `data_4df904 + 0x30d`
