...
- the debug `Segments` view now carries all public authored attachment families through that scaffold, including the nonlinear kind-`42` family, and the gameplay runner now consumes built templates for live attachment progression, camera basis, natural-end exit pose, and a first width-based side-exit rule; the remaining gaps are the installed runtime attachment bank, family-specific nonlinear semantics, and special exits like `SUPERTRAMP`
