...
- the gameplay runner now also consumes those built templates for live attachment progression, world pose, camera forward/up, natural-end exit pose, a first width-based side-exit rule, and a dedicated `SUPERTRAMP` launch exit; installed attachment entry now splits a native-shaped current-row begin from the recovered built-geometry sweep instead of reusing the older source-row fallback
