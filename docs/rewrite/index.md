...
- the debug `Segments` view now carries all public authored attachment families through that scaffold, including the nonlinear kind-`42` family, and the gameplay runner now consumes built templates for live attachment progression, curved-surface camera basis, natural-end exit pose, and a first width-based side-exit rule; the remaining gaps are the installed runtime attachment bank and family-specific exits like `SUPERTRAMP`
