...
- the intro and credits crawl now also follows the recovered tilted text-plane setup from `initialize_intro_text_screen`, including the shared duration scroll, world-sized image directives, projected textured quads, and `Esc` or click skip behavior, while exact screen-transition semantics still remain unresolved
