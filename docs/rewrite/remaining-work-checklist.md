...
- [ ] Finish moving any remaining post-overlay/final-loss/thanks-for-playing updates onto that same explicit bridge
  ...
  - remaining gap: the current port still maps challenge-side rebuild returns onto the existing `Challenge Mode` menu stand-in instead of the literal challenge-setup controller
