...
- [ ] Finish moving any remaining post-overlay/final-loss/thanks-for-playing updates onto that same explicit bridge
  ...
  - current Zig consequence: high-score replay launches now map postal/challenge result exits through the confirmed persistent `0x1a` destroy-return lane, while route-map best-trial replay launches stay on the transient `0x1b` rebuild-return path; transient postal replay final loss also still follows the native `0x1a -> owner 2` New Game override because `complete_subgame` never seeds app byte `+0x30d` while `selected_level_record_active` is set
  - remaining gap: the exact replay-backed abandon / overlay routing for those now-distinguished persistent vs transient launch families, and the exact non-selected-record postal final-loss use of `data_4df904 + 0x30d`
