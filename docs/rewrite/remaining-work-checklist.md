...
- [ ] Port the minimum player motion slice needed to tighten attachment-exit retirement to the native lanes (harvested from the retired 2026-04 infrastructure plan)
  ...
  - deferred follow-ups: 0x43c06d re-snap needs a tile `flags_3d` + global flag 2 mirror; 0x43c3ea trampoline needs a `cell_y` accessor (current proxy fires too eagerly; corrected 06-12: the bounce SETS vy = rate*0.3 rather than flipping velocity, snaps y to anchor+0.49, and sets the +0x1e4 bounce byte that gates the vz drag); 0x43ce75 needs `position_y` plus a `jetpack_gauge.state == 1` mirror; full `velocity.x/z` model and the ±4 lateral clamp come last
