...
| 6 | golb / projectiles | search_path + init_path_follow 100%; update_golb_ai scratch 20.8% structure-complete (full semantics); traverse_path_follow_golb = boss sibling | 🚧 | 🚧 `native/golb.zig` 06-13 (band gravity, homing, trails, spin, slug deflection, contact band, path-entry predicate — all tested) | 🚧 live lanes ROUTED 06-13: band gravity, 3D slug deflection (2D approximation collapsed), contact band, smoke fraction; REMAINING: homing target acquisition, golb FollowState riding (traverse_path_follow_golb ≈ updateTrackAttachmentFollowState), trail-sprite triple | · |
