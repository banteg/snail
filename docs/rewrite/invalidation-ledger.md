...
## 2026-06-13 - Garbage attachment-row spawn projection

- invalidated claim: runtime garbage slots can seed their live collision body
  directly from the flat runtime cell world position; any later visual or bob
  helper is presentation-only
- replacement evidence: `spawn_track_garbage_hazard` @ 0x43da80 seeds the
...
- replacement evidence: matched native source (tools/match) shows no family
  ...
  `try_enter_track_attachment_from_swept_motion` (95.78%)
...
- invalidated claim: the 0x214-byte child at `Player +0x2750` is only a neutral jetpack-gauge composite, and its final call is semantically `cRSubGame::spawn_track_speedup`
