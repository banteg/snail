...
- the intro and credits crawl now also uses the recovered tilted text-plane layout, duration-driven shared scroll distance, and world-unit image sizing from `initialize_intro_text_screen`, though image quads are still drawn with a simpler screen-space approximation than the original runtime
