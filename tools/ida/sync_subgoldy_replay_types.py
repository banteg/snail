...
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_subgoldy_replay_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply focused subgoldy replay and ghost-record types to IDA."
    )
    parser.add_argument(
        "--ida-bin",
    ...
    parser.add_argument(
        ...
        help="Path to the canonical subgame owner type header.",
    )
...
def main() -> int:
    ...
    exit_code, log_text = run_ida_script(
        ...
        log_stem="sync-subgoldy-replay-types",
    )
