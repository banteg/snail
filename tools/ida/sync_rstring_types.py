...
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_rstring_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Replay the mobile-proven RString.o contracts in IDA 9.4."
    )
    parser.add_argument(
        "--ida-bin",
...
def main() -> int:
    ...
    if not IDAPYTHON_SCRIPT_PATH.is_file():
        raise FileNotFoundError(
            f"IDAPython RString sync script not found: {IDAPYTHON_SCRIPT_PATH}"
        )
