...
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_parcel_bucket_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Preview on a temporary database, then replay parcel-bucket ownership "
            "to IDA."
        )
    )
    parser.add_argument(
...
def main() -> int:
    ...
    if not IDAPYTHON_SCRIPT_PATH.is_file():
        raise FileNotFoundError(
            f"IDAPython parcel-bucket sync script not found: {IDAPYTHON_SCRIPT_PATH}"
        )

    with tempfile.TemporaryDirectory(prefix="snail-ida-parcel-preview-") as temp_dir:
        ...
        preview_exit_code, preview_log_text = run_ida_script(
            ...
            log_stem="preview-parcel-bucket-types",
        )
    ...
    exit_code, log_text = run_ida_script(
        ...
        db_path=db_path,
        script_args=[str(header_path)],
        log_stem="sync-parcel-bucket-types",
    )
