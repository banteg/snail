...
OWNER_TYPE_ALIASES = (("SnailSkin", "cRSnailSkin", 0x20),)
...
def _owner_layout_readback() -> dict[str, object]:
    ...
    if snail_embed != EXPECTED_SNAIL_EMBED:
        failures.append(
            {
                "selector": "Snail.snail_skin",
                "reason": "embedded_owner_mismatch",
                "expected": EXPECTED_SNAIL_EMBED,
            ...
            json.dumps(
                {
                    ...
                    "failed": [{"reason": "noncanonical_snail_skin_header"}],
                },
                indent=2,
            )
        )
...
def main() -> None:
    argv = list(idc.ARGV)
    if len(argv) != 2:
        print("usage: apply_snail_skin_types.py <header-path>", file=sys.stderr)
        ida_pro.qexit(2)
        return
