...
DEFAULT_HEADER_PATH = REPO_ROOT / "analysis/headers/ida_high_score_bank_types.h"
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_high_score_bank_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply recovered high-score bank types, prototypes, and names to an IDA database."
    )
    parser.add_argument(
        "--ida-bin",
    ...
    parser.add_argument(
        ...
        help="Path to the checked-in high-score bank type header.",
    )
...
def main() -> int:
    ...
    if not header_path.is_file():
        raise FileNotFoundError(f"high-score bank type header not found: {header_path}")
    ...
    exit_code, log_text = run_ida_script(
        ...
        log_stem="sync-high-score-bank-types",
    )
