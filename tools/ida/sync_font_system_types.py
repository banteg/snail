...
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_font_system_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply recovered font runtime owners to an IDA database."
    )
    parser.add_argument(
        "--ida-bin",
...
def main() -> int:
    ...
    if not IDAPYTHON_SCRIPT_PATH.is_file():
        raise FileNotFoundError(
            f"IDAPython font-system sync script not found: {IDAPYTHON_SCRIPT_PATH}"
        )
    ...
    exit_code, log_text = run_ida_script(
        ...
        log_stem="sync-font-system-types",
    )
