...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply recovered RShell archive/file ownership to an IDA database."
    )
    parser.add_argument(
        "--ida-bin",
    ...
    parser.add_argument(
        ...
        help="Path to the checked-in RShell archive/file type header.",
    )
...
def main() -> int:
    ...
    if not header_path.is_file():
        raise FileNotFoundError(f"archive/file type header not found: {header_path}")
