...
DEFAULT_HEADER_PATH = REPO_ROOT / "analysis/headers/object_render_types.h"
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_object_render_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply recovered object render/animation structs and trusted prototypes to an IDA database."
    )
    parser.add_argument(
        "--ida-bin",
    ...
    parser.add_argument(
        ...
        help="Path to the checked-in object render type header.",
    )
...
def main() -> int:
    ...
    if not header_path.is_file():
        raise FileNotFoundError(f"object render type header not found: {header_path}")
    ...
    exit_code, log_text = run_ida_script(
        ...
        log_stem="sync-object-render-types",
    )
