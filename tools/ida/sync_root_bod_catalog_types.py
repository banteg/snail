...
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_root_bod_catalog_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply the constructor-proven root BOD catalog owner to IDA."
    )
    ...
    parser.add_argument(
        ...
        help="Shared root BOD catalog ownership header.",
    )
...
def main() -> int:
    ...
    if not header_path.is_file():
        raise FileNotFoundError(f"root BOD catalog type header not found: {header_path}")
    ...
    exit_code, log_text = run_ida_script(
        ...
        log_stem="sync-root-bod-catalog-types",
    )
