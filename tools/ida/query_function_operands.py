...
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/inspect_function_operands.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "List IDA instruction operands, numeric values, and operand indices "
            "inside selected functions."
        )
    )
    parser.add_argument("selectors", nargs="+", help="Function names or addresses.")
    parser.add_argument(
        "--match",
        default="",
        help="Only emit instructions matching this case-insensitive regex.",
    )
    parser.add_argument(
        "--ida-bin",
...
def main() -> int:
    ...
    exit_code, log_text = run_ida_script(
        ...
        log_stem="inspect-function-operands",
    )
