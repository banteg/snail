...
OWNER_TYPE_ALIASES = (("TimesUp", "cRTimesUp", 0x10),)
...
def _owner_layout_readback() -> dict[str, object]:
    ...
    if subgame_embed != EXPECTED_SUBGAME_EMBED:
        failures.append(
            {
                "selector": "cRSubGame.times_up",
                "reason": "embedded_owner_mismatch",
                "expected": EXPECTED_SUBGAME_EMBED,
            ...
            json.dumps(
                {
                    ...
                    "failed": [{"reason": "noncanonical_times_up_header"}],
                },
                indent=2,
            )
        )
...
def main() -> None:
    argv = list(idc.ARGV)
    if len(argv) != 2:
        print("usage: apply_times_up_types.py <header-path>", file=sys.stderr)
        ida_pro.qexit(2)
        return
