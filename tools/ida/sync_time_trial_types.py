...
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_time_trial_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply the exact Windows cRTimeTrial ownership lane to IDA."
    )
    parser.add_argument(
        "--ida-bin",
        help=(
            "Path to the IDA headless binary "
            "(defaults to the first idat*/ida* on PATH)."
        ),
    )
    parser.add_argument(
        "--db",
    ...
    parser.add_argument(
        ...
        help="Path to the checked-in cRTimeTrial type header.",
    )
...
def main() -> int:
    ...
    if not header_path.is_file():
        raise FileNotFoundError(f"cRTimeTrial type header not found: {header_path}")
    ...
    exit_code, log_text = run_ida_script(
        ...
        log_stem="sync-time-trial-types",
    )
