...
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_rmath_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Replay the mobile-proven Windows RMath contracts in IDA."
    )
    parser.add_argument(
        "--ida-bin",
        help="Path to IDA 9.4 headless binary.",
    )
    parser.add_argument(
        "--db",
        type=Path,
...
def main() -> int:
    ...
    if not IDAPYTHON_SCRIPT_PATH.is_file():
        raise FileNotFoundError(
            f"IDAPython RMath sync script not found: {IDAPYTHON_SCRIPT_PATH}"
        )
