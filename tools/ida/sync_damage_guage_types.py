...
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_damage_guage_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply the exact Windows cRDamageGuage ownership lane to IDA."
    )
    parser.add_argument(
        "--ida-bin",
    ...
    parser.add_argument(
        ...
        help="Path to the checked-in damage-gauge type header.",
    )
...
def main() -> int:
    ...
    if not header_path.is_file():
        raise FileNotFoundError(f"damage-gauge type header not found: {header_path}")
    ...
    exit_code, log_text = run_ida_script(
        ...
        log_stem="sync-damage-guage-types",
    )
