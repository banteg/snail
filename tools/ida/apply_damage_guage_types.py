...
OWNER_TYPE_ALIASES = (("DamageGuage", "cRDamageGuage", 0x2C),)
            ...
            json.dumps(
                {
                    ...
                    "failed": [{"reason": "noncanonical_damage_guage_header"}],
                },
                indent=2,
            )
        )
...
def main() -> None:
    argv = list(idc.ARGV)
    if len(argv) != 2:
        print("usage: apply_damage_guage_types.py <header-path>", file=sys.stderr)
        ida_pro.qexit(2)
        return
