...
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_gui_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply the exact Windows cRGUI ownership lane to IDA."
    )
    parser.add_argument(
        "--ida-bin",
        help=(
    ...
    parser.add_argument(
        ...
        help="Path to the checked-in cRGUI type header.",
    )
...
def main() -> int:
    ...
    if not header_path.is_file():
        raise FileNotFoundError(f"cRGUI type header not found: {header_path}")
