...
DEFAULT_HEADER_PATH = REPO_ROOT / "analysis/headers/frontend_replay_types.h"
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_frontend_replay_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply recovered cRBorder draw, cRIntro, and front-end replay ownership to an IDA database."
    )
    parser.add_argument(
        "--ida-bin",
    ...
    parser.add_argument(
        ...
        help="Path to the checked-in front-end replay type header.",
    )
...
def main() -> int:
    ...
    if not header_path.is_file():
        raise FileNotFoundError(f"front-end replay type header not found: {header_path}")
    ...
    exit_code, log_text = run_ida_script(
        ...
        log_stem="sync-frontend-replay-types",
    )
