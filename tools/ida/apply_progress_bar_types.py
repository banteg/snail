...
OWNER_TYPE_ALIASES = (("ProgressBar", "cRProgressBar", 0x01),)
            ...
            json.dumps(
                {
                    ...
                    "failed": [{"reason": "noncanonical_progress_bar_header"}],
                },
                indent=2,
            )
        )
...
def main() -> None:
    argv = list(idc.ARGV)
    if len(argv) != 2:
        print("usage: apply_progress_bar_types.py <header-path>", file=sys.stderr)
        ida_pro.qexit(2)
        return
