...
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_cheat_state_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply the exact cRCheat global owner and lifecycle ABIs to IDA."
    )
    parser.add_argument(
        "--ida-bin",
    ...
    parser.add_argument(
        ...
        help="Path to the checked-in CheatState type header.",
    )
...
def main() -> int:
    ...
    exit_code, log_text = run_ida_script(
        ...
        log_stem="sync-cheat-state-types",
    )
