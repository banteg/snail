...
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_voice_manager_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply the exact VoiceSet and VoiceManager ownership lane to IDA."
    )
    parser.add_argument(
        "--ida-bin",
    ...
    parser.add_argument(
        ...
        help="Path to the checked-in VoiceManager type header.",
    )
...
def main() -> int:
    ...
    if not header_path.is_file():
        raise FileNotFoundError(f"VoiceManager type header not found: {header_path}")
    ...
    exit_code, log_text = run_ida_script(
        ...
        log_stem="sync-voice-manager-types",
    )
