...
DEFAULT_HEADER_PATH = REPO_ROOT / "analysis/headers/frame_renderer_types.h"
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_frame_renderer_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply the recovered frame-renderer ABI and workspace types to IDA."
    )
    parser.add_argument(
        "--ida-bin",
    ...
    parser.add_argument(
        ...
        help="Path to the checked-in frame-renderer type header.",
    )
...
def main() -> int:
    ...
    if not header_path.is_file():
        raise FileNotFoundError(f"frame-renderer type header not found: {header_path}")
    ...
    exit_code, log_text = run_ida_script(
        ...
        log_stem="sync-frame-renderer-types",
    )
