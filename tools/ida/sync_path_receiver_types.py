...
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_path_receiver_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply the focused exact Path receiver ABIs to IDA."
    )
    parser.add_argument(
        "--ida-bin",
    ...
    parser.add_argument(
        ...
        help="Path to the canonical Path owner type header.",
    )
...
def main() -> int:
    ...
    exit_code, log_text = run_ida_script(
        ...
        log_stem="sync-path-receiver-types",
    )
