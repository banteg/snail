...
IDAPYTHON_SCRIPT_PATH = (
    REPO_ROOT / "tools/ida/apply_win32_window_state_types.py"
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply the recovered Win32 window-state owners to IDA."
    )
    parser.add_argument(
        "--ida-bin",
...
def main() -> int:
    ...
    exit_code, log_text = run_ida_script(
        ...
        log_stem="sync-win32-window-state-types",
    )
