...
IDAPYTHON_SCRIPT_PATH = REPO_ROOT / "tools/ida/apply_player_lifecycle_types.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply the focused Player lifecycle and effects ownership ABIs to IDA."
    )
    parser.add_argument(
        "--ida-bin",
    ...
    parser.add_argument(
        ...
        help="Path to the canonical Player owner type header.",
    )
...
def main() -> int:
    ...
    exit_code, log_text = run_ida_script(
        ...
        log_stem="sync-player-lifecycle-types",
    )
