...
function summarizeCell(cellPtr, getTrackCellRowIndex, gamePtr) {
  ...
  if (derived !== null && derived.storage === 'grid') {
    return {
      ...
      render_object: hex(safeReadPointer(cell, 0x24)),
      render_alpha: safeReadFloat(cell, 0x34),
      fringe_back: hex(safeReadPointer(cell, 0x50)),
...
function summarizeAttachmentTemplate(templatePtr) {
  ...
  return {
    ...
    is_mirrored_x: boolFlag(safeReadU8(template, 0x3c)),
    side_exit_mode: safeReadU32(template, 0x40),
    segment_count: safeReadU32(template, 0x44),
    row_span_count: safeReadU32(template, 0x48),
    segment_count_f32: safeReadFloat(template, 0x4c),
    width_or_scale: safeReadFloat(template, 0x50),
    width_cells: safeReadU32(template, 0x54),
    primary_samples: hex(safeReadPointer(template, 0x58)),
    secondary_samples: hex(safeReadPointer(template, 0x5c)),
    installed_heading_delta: safeReadFloat(template, 0x98),
    has_entry_mesh_transition: boolFlag(safeReadU8(template, 0x9c)),
    entry_transition_strip_mesh: hex(safeReadPointer(template, 0xa0)),
    entry_base_strip_mesh: hex(safeReadPointer(template, 0xa4)),
  };
}
...
function summarizeFollowState(followStatePtr, getTrackCellRowIndex) {
  ...
  return {
    ...
    orientation_a: safeReadFloat(followState, 0x18),
    orientation_b: safeReadFloat(followState, 0x1c),
    orientation_up: safeReadVec3(followState, 0x20),
    ...
    Interceptor.attach(fromVa(module, VA.death_handoff_via_cutscene), {
      onEnter() {
        ...
        emit('death_handoff_cutscene', {
          ...
          resurrect_active: boolFlag(safeReadU8(player, 0x84)),
          exit_pending: boolFlag(safeReadU8(player, 0x41d)),
          gate_a: boolFlag(safeReadU8(player, 0x44c)),
          gate_b: boolFlag(safeReadU8(player, 0x44d)),
          app: summarizeAppState(getAppPtr(module)),
