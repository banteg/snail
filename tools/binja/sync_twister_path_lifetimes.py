...
# BuildTwisterA and BuildTwister2A were emitted from the same source skeleton,
# so both carry the same MLIL variable identities. The orientation loop keeps
# exact borrowed Vec3 fields plus reloadable current-sample bases. The terminal
# delta stores point at PathTemplateSample::delta_dir_to_next, while the mesh
# loops retain one complete sample and two complete ObjectFaceQuad records.
TWISTER_PATH_LIFETIME_SPECS = (
    (746, 68, "primary_up", "Vec3*"),
    (866, 66, "primary_sample_cursor", "PathTemplateSample*"),
    (
        897,
        66,
        "primary_sample_cursor_reloaded",
        "PathTemplateSample*",
    ),
    ...
    (
        1114,
        66,
        "secondary_sample_cursor_reloaded",
        "PathTemplateSample*",
    ),
...
TWISTER_PATH_USER_VAR_UPDATES = tuple(
    (
        function_name,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Replay the sample-vector, terminal-delta, and facequad lifetimes "
            "shared by the two twister path constructors."
        )
    )
    parser.add_argument(
        "--target",
...
def verify_owner_layouts(target: str) -> dict[str, object]:
    ...
    if mismatches:
        raise RuntimeError(
            "canonical twister path ownership layout is not current:\n"
            + "\n".join(mismatches)
        )
    return {
        "op": "verify_twister_path_owner_layouts",
        "status": "verified",
