...
| `update_subgame` | `0x438b90` | 71.32%, structure-first | Outer gameplay state machine, active row scan, bridge opcodes. | States `0/1/2/3/4/7`, runtime rows, and the embedded `Player` owner are covered. The selected-level handoff occupies the native cold tail, projected-ring addresses recover native short-lived ownership, and the time-trial record base now reuses its native pointer; remaining work is state-1 build/destroy plus residual ring/HUD register scheduling. |
