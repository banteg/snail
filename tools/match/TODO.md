...
| `update_golb_ai` | `0x414820` | 24.36%, structure complete | Projectile flight, path entry, homing, trails, slug/garbage hit gates, wall impact. | Live-state gate, path-follow output copies, and non-follow position pointer staging now better match native; continue around the `0x70` frame gap, trail/smoke offset staging, direction update locals, slug deflection, and duplicated returns documented in NOTES. |
