...
| `spawn_track_garbage_hazard` | `0x43da80` | 89.75%, source-shaped | Seeds garbage scale, sprite variant, projected body position, active list link, and slot owner. Recent port fix depends on this path. | Pool scan and slot/player/scale setup now match native shape; continue source-shaping around projection x87/register order. |
