...
| `spawn_garbage_smoke_particle` | `0x43d5a0` | 84.56%, source-shaped | Smoke-effect producer called by `update_garbage_hazard`; owns render/effects gate, sprite setup, subgame-rate lifetime lanes, and velocity scaling. | Return is now modeled as ignored/void; remaining residual is color and velocity copy scheduling documented in NOTES. |
...
| `initialize_ring_or_special_effect_particles` | `0x43e470` | 99.35%, pinned | Allocates and seeds the ten child halo sprites for ring/special-effect parents, including particle phase/radius and star sprite kind. | Semantics and child/parent layouts are pinned; remaining residual is phase-step/parent store scheduling documented in NOTES. |
| `emit_ring_star_shower` | `0x43e690` | 74.42%, source-shaped | Small star burst producer called by the child halo updater; confirms parent star sprite id and particle sprite/phase/radius lanes. | Dword-shaped position copy now matches the native return relationship; remaining stack-frame and velocity staging residual is documented in NOTES. |
...
| `update_ring_or_special_effect_parent` | `0x43e830` | 86.35%, source-shaped | Parent virtual updater for ring/special-effect records: cadence, normal orbit, collapse/expand transitions, active-list removal, and child sprite teardown. | State-3 collapse now uses a real `Vector3 delta` and immutable target copy; remaining residual is switch/x87 stack shape plus the named jump-table audit documented in NOTES. |
...
| `direct3d_renderer_set_cull_mode` / `set_cull_mode` / `set_blend_mode` | `0x411700` / `0x4129f0` / `0x412d00` | 100% / 100% / 81.52%, source-shaped | Object-render state helpers behind `render_object`: cull mode maps to Direct3D render state `0x16`, and blend presets map authored ids through the compiler lookup table to render states `0x1b/0x13/0x14`. | Cull wrapper pair is exact. Blend helper is source-shaped with real authored-id mapping; remaining residual is VC6 tail-sharing owner for duplicate `src=5, dest=6` cases plus target-vs-scratch table label operands. Use these as anchors before expanding `render_object`. |
...
| `get_or_append_object_texture_group_vertex` / `build_object_texture_group_buffers` | `0x413bb0` / `0x413d50` | 45.85% / 20.20%, relationship-first | Build-time grouped vertex/index-buffer construction behind object rendering. Confirms temporary grouped vertex stream, texture-group end indices, and object render buffer fields. | Clean masks. Append helper pins `data_5031bc` cursor, `data_5031c4` 0x1c scratch records, UV flip, and flag-dependent de-dup keys. Builder pins `+0xcc/+0xd0/+0xd4`, `+0xc0/+0xc8/+0xd8`, `+0xc4`, facequad flag `0x80`, and `+0x6c` as cumulative group ends. Exact `replace_object_group_texture_refs` now confirms `Object +0xd0` is also updated after source facequad texture swaps. |
...
| `calc_object_texture_groups` | `0x4303f0` | 28.83%, relationship-first | Two-pass producer for facequad texture-group boundaries before `build_object_texture_group_buffers`. | Pins `Object +0x6c` as cumulative facequad end indices, `+0x64/+0x68` ownership through `request_object_texture_groups`, facequad texture refs at `+0x0c`, split flag `0x10`, and object flag `0x400` side effects on facequad/texture flags. Residual is native register ownership for pass/group/face/texture locals. |
| `request_object_animation` | `0x430a70` | 41.24%, relationship-first | X-mesh animation owner that builds generated frame arrays from 0x80-byte keyframe records. | Pins keyframe object pointer `+0x24`, keyframe frame number `+0x7c`, target animation pointer `Object +0xbc`, generated vertices from `Object +0x38`, generated facequad normals from `Object +0x60`, and 0x14-byte `ObjectAnimation` / 8-byte `ObjectAnimationFrame` layouts. Residual is stack-local/register scheduling in interpolation and the CRT floor helper operand. |
| `replace_object_group_texture_refs` / `replace_object_list_texture_refs` / `select_level_track_texture_set` | `0x4145c0` / `0x430d90` / `0x410730` | 100% / 51.85% / 76.19% | Level texture-set selection rewrites texture refs across the global object list, keeping source facequads and already-built grouped render tables in sync. | Group-table helper is exact and pins `flags & 0x80000`, `Object +0x64`, and `Object +0xd0`. List helper and selector are relationship-first; keep them as mutators because the sole selector caller ignores `eax` and the replacement helper's return register is incidental. |
| `rstrcpy_checked_ascii` / `strings_equal_case_insensitive` / `find_case_insensitive_substring` / `advance_to_next_crlf_line` / `strings_equal_case_insensitive_path` / `parse_next_signed_int` | `0x44e5b0` / `0x431dc0` / `0x44e600` / `0x44e690` / `0x44e6c0` / `0x44e710` | 100% / 29.13% / 25.71% / 79.07% / 100% / 28.87% | Shared resource string and parser helpers used by texture, object, X-mesh, level, segment, landscape, voice, and music loaders. | Copy helper and path comparator are exact; `advance_to_next_crlf_line` is distinct from the older exact `skip_to_next_line`; remaining partials are relationship-first scratches with documented register/control-flow residuals. |
...
| `add_arcade_high_score` / `add_survival_high_score` | `0x4176a0` / `0x417780` | 65.69% / 66.67%, source-shaped | Postal/Arcade and Challenge/Survival result persistence helpers called by `complete_subgame`; own top-ten insertion and high-score entry frontend arming. | Shared `HighScoreBank` layout pins postal, survival, time-trial route, current-result, and survival-pending record windows. `HighScoreRecord` now has a 0x14-byte `player_name`, `runtime_seed +0x68`, `completion_count +0x6c`, and 21600 replay records at `+0x70` with flags at entry `+0x04`. Residuals are rank-scan register ownership and frontend global-store scheduling; do not force with raw offset macros. |
| `initialize_high_score_entry` | `0x417a70` | 85.00%, source-shaped | Seeds one replay/high-score record before a run or blank table entry; strongest evidence for the record prefix and replay-entry layout. | Layout is pinned: `TimerCounters` union at `+0x08`, runtime flags at `+0x38`, `runtime_seed +0x68`, replay records from `+0x70`, and flags at record `+0x04`. Remaining residual is post-`rstrcpy` argument load/store scheduling for mode, route, and flags; do not add artificial barriers. |
| `commit_high_score_entry_into_top_ten` | `0x417af0` | 38.81%, relationship-first | Commits edited name-entry display rows into the active persistent top-ten bank. | Promotes `HighScoreRecordView` at `game+0x6ffae0`: active bank pointer/count plus `name_submit_records` at view `+0x17c108`. Remaining residual is register allocation for rank/source pointer in the copy loop. |
