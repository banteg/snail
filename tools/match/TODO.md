...
| `get_or_append_object_texture_group_vertex` / `build_object_texture_group_buffers` | `0x413bb0` / `0x413d50` | 63.20% / 20.20%, source-shaped | Build-time grouped vertex/index-buffer construction behind object rendering. Confirms temporary grouped vertex stream, texture-group end indices, and object render buffer fields. | Clean masks. Append helper pins `data_5031bc` cursor, `data_5031c4` 0x1c scratch records, UV flip, `Vector3` source-position stack copy, native `scratch + 4` de-dup cursor, and flag-dependent de-dup keys. Builder pins `+0xcc/+0xd0/+0xd4`, `+0xc0/+0xc8/+0xd8`, `+0xc4`, facequad flag `0x80`, and `+0x6c` as cumulative group ends. Exact `replace_object_group_texture_refs` now confirms `Object +0xd0` is also updated after source facequad texture swaps. Remaining append-helper residual is branch/block layout plus append-tail x87 scheduling; raw-offset append regresses with a masked global mismatch. |
