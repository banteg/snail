...
| `update_golb_ai` | `0x414820` | 27.05%, structure complete | Projectile flight, path entry, homing, trails, slug/garbage hit gates, wall impact. | Live-state gate, path-follow output copies, non-follow position pointer staging, kind-0 gravity ordering, direction delta locals, horizontal slug-deflection normalization, and slug kind compare order now better match native; continue around the `0x70` frame gap, trail/smoke offset staging, reflected velocity store order, and duplicated returns documented in NOTES. |
