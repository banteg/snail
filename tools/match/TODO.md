...
|---|---:|---|---|---|
| `update_garbage_hazard` | `0x43f200` | 74.67%, source-shaped | Owns garbage active/burst states, smoke cadence, nuke reaction, and the final live body position that collision/projectile probes sample. | State-3 integration/source copy is closer to native; continue only around the remaining `0x1c` frame gap, burst x87 staging slots, and case-1 vector-copy register order documented in NOTES. |
...
| `spawn_track_garbage_hazard` | `0x43da80` | 92.58%, source-shaped | Seeds garbage scale, sprite variant, projected body position, active list link, and slot owner. Recent port fix depends on this path. | Pool scan, setup, staged x/z load order, and initial y x87 load now match native shape; remaining gap is projection register spill order. |
