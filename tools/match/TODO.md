...
| `initialize_high_score_entry` | `0x417a70` | 97.50%, source-shaped | Seeds one replay/high-score record before a run or blank table entry; strongest evidence for the record prefix and replay-entry layout. | Layout is pinned: `TimerCounters` union at `+0x08`, `runtime_build_flags +0x38`, `runtime_build_seed +0x68`, replay records from `+0x70`, and flags at record `+0x04`. Remaining residual is the post-`rstrcpy` active/flags store-order swap; do not add artificial barriers. |
