...
| `update_player_movement_flags` | `0x43a1a0` | 100% | Maps the movement selector to the Golb/fire movement flag mask, fire cadence step, and presentation refresh gate. | Done; selector table, cadence constants, and `set_snail_weapon` gate are proof-grade, with the jump-table audit clean. |
...
| `calc_path_length_z` | `0x4217b0` | 55.52%, structure complete | Golb path-follow riding; sibling of `update_track_attachment_follow_state`. | Scalar/base declaration-order trials are exhausted; continue only with stronger evidence around overflow block placement, matrix-copy layout, and terminal/side-exit shot-position ownership documented in NOTES. |
| `create_golb` | `0x415280` | 36.08%, pinned | Seeds projectile kind, velocity/spread, homing target, and RNG stream consumers. | Attachment-family z-floor semantics are corroborated by Windows and iOS implementations, and stable projectile/list/vapour/follow owners are typed. Residuals are compiler shared-label and stack-vector scheduling with documented rejected trials; resume only with new source or localized provenance. |
...
| function | address | current | why it matters | next matching move |
|---|---:|---|---|---|
| `update_subgoldy` | `0x43b120` | 72.51%, structure complete | Master player motion, replay codec, fire lane, completion handoff, attachment-exit clears. | Continue slice-first: grounded/trampoline clear lanes, projectile/replay consumers, then residual register-shape cleanup. |
...
| `update_squidge` | `0x4449c0` | 100% | Damped y/z presentation spring driven by exact `start_squidge_y` and `start_squidge_z`, used by the player motion presentation. | Done; use with exact `start_squidge_y` and `start_squidge_z` as the player presentation spring source of truth. |
...
| `update_track_attachment_follow_state` | `0x420cb0` | 69.38%, semantics pinned | Attachment stepping, natural end, side exit, supertramp launch. | Golf only with source-plausible staging; do not regress routed mirror semantics. |
| `project_position_onto_track_attachment` | `0x4444b0` | 88.68%, source-shaped | Shared projection for garbage/slug/parcel and attachment-row actors. | Kind-42 helper is now exact as a `PathTemplate` member call; continue only around branch-label/local-transform offsets and non-kind42 stack-slot/register residuals documented in NOTES. |
...
| function | address | current | why it matters | next matching move |
|---|---:|---|---|---|
| `handle_subgoldy_collisions` | `0x444cf0` | 52.85%, structure complete | Contact gates for salt, sub-lazer, garbage, slug, rings, health, jetpack, parcel. | The root owner now exposes exact `SlugHazardPool[8]`, `SubLazerPool[20]`, and `SaltHazardPool[40]` fields; the 0xec slug stride and real helper methods are promoted, and all 86 masks are clean. Remaining proof-polish is local staging, mask-register reuse, and per-loop delta/probe shape. |
...
| `spawn_track_ring_or_special_effect` | `0x43df10` | 53.45%, evidence-first | Parent allocator/spawner for two ring/special-effect records embedded in `SubgameRuntime`; seeds parent position, active phase, borrowed owner/rate views, active-list links, and child particles. | Typed pool/slot/transform staging is accepted; remaining residual is grouped switch paths versus native per-kind RNG tags (`RR..RR13`) and the honest jump-table-content mismatch documented in NOTES. |
...
| `emit_ring_star_shower` | `0x43e690` | 100% | Small star burst producer called by the child halo updater; confirms parent star sprite id and particle sprite/phase/radius lanes. | Done; the semantic `Vector3` position copy and star sprite/radius lanes are proof-grade anchors for ring/special-effect presentation. |
...
| `update_ring_or_special_effect_parent` | `0x43e830` | 98.21%, source-shaped | Parent virtual updater for embedded ring/special-effect records: cadence, normal orbit, collapse/expand transitions, non-owning active-list removal, and manager-owned child sprite teardown. | Biased target `Vector3` staging and indexed embedded-child walks are accepted; only the state-3 temporary schedule, one duplicated list-anchor load, and the compiler-local jump-table symbol remain. |
| `initialize_subgame` | `0x4374b0` | 100%, audit-only | Establishes the root ownership graph: embedded high-score banks/snapshots, BorderManager widget handles, runtime-cell fringe handles, player/warning state, challenge setup, and galaxy route. | All `396/396` instructions match; the sole remaining audit item is one compiler-local switch-table symbol versus the curated target table name. |
| `wall2_emitter_maybe_fire_sub_lazer` | `0x439d50` | 46.77%, pinned | Wall2 fire cadence and 4% RNG gate. | Tile dispatch now uses the native compare-ladder family and unsigned lane conversion; remaining gaps are frame/register scheduling around wall2 vector setup and skirt-color sync documented in NOTES. |
...
| `spawn_salt_hazard` | `0x441560` | 100% | Salt slot layout, velocity byte-poke bug, yaw RNG. | Done; the free-scan loop, low-byte velocity.z poke, random yaw, and list add-after shape are proof-grade. |
...
| `update_salt_hazard` | `0x441c10` | 100% | Salt fade/update state installed by `initialize_salt_hazard_runtime`. | Done; use with exact `spawn_salt_hazard` and `deactivate_salt_hazard` as the salt lifecycle source of truth. |
...
| `update_tip` | `0x448c40` | 100% | Tip slot runtime: suspend hide/unhide, OK/Disable click consumption, auto-dismiss, widget teardown, and frontend owner restore. | Done; use with exact `initialize_tip`, `uninit_tip`, and `enqueue_tip_message` as the tip slot lifecycle anchor. |
| `initialize_tip` | `0x448a40` | 84.42%, source-shaped | Tip slot initializer: allocates the main text widget, optional OK/Disable buttons, default tip definition, dismiss timing, and frontend owner capture. | The second argument now matches native as a hide-Disable gate, and direct button y expressions recover native outgoing-slot staging; remaining residual is widget field-load/order and final owner-restore scheduling documented in NOTES. |
...
| `refresh_object_vertex_buffer` | `0x412250` | 60.81%, relationship-first | Private `render_object` helper that refreshes grouped D3D vertex-buffer positions, animation frame vertex/facequad-normal sources, optional distortion, and static facequad UVs. | Shared `Object` ownership pins animation at `+0xbc`, render buffers at `+0xc0`, grouped vertex count at `+0xc4`, facequads at `+0x5c`, facequad normals at `+0x60`, and distortion at `+0x80`. A borrowed active-vertices view recovers prefix 41/139; remaining residual is the offset-loop register schedule. |
| `render_object` / `render_object_toon` | `0x4126c0` / `0x4123e0` | 80.31% / 57.76%, relationship-first | Frame-time object renderer and private toon/outline tail helper; consumes grouped vertex/index buffers, texture groups, tint state, cull/blend helpers, projection globals, and edge records. | Clean masks. `render_object` now matches the native zero-vertex result, exact two-sided pass filter, and repeated borrowed texture-ref access; its first residual is loop register ownership. `render_object_toon` corrects the gate to `flags & 0x4000`, consumes `+0xd8`, `+0x70/+0x74`, and `+0x60`, and corrects edge bit 1 as unconditional emission. |
| `get_or_append_object_texture_group_vertex` / `build_object_texture_group_buffers` | `0x413bb0` / `0x413d50` | 63.20% / 43.00%, source-shaped | Build-time grouped vertex/index-buffer construction behind object rendering. Confirms temporary grouped vertex stream, texture-group end indices, and object render buffer fields. | Clean masks. Append helper pins `data_5031bc` cursor, `data_5031c4` 0x1c scratch records, UV flip, `Vector3` source-position stack copy, native `scratch + 4` de-dup cursor, and flag-dependent de-dup keys. Builder has one shared `void(Object*)` declaration and retains `+0xc0/+0xc8/+0xcc/+0xd0/+0xd4`; early group/index counter lifetimes recover one more native instruction/mask. Its residual is the honest `0x18` versus native `0x28` frame plus texture-group loop pointer ownership. |
...
| `get_or_create_texture_ref` | `0x44e810` | 100% | Shared texture-reference list lookup/allocator behind sprite texture registration, mesh/path textures, and sprite runtime texture refs. | Done; `TextureRefList` and `TextureRef` stride/name/slot fields are proof-grade and can anchor texture registry consumers. |
...
| function | address | current | why it matters | next matching move |
|---|---:|---|---|---|
| `place_parcels_on_track` | `0x4438e0` | 26.30%, structure-first | Parcel RNG stream, placement banks, attachment-row projection. | Row payload lanes are documented at row +0x90..+0x9b; continue around frame/register bank scan and ordered-compaction shape. |
...
| `request_object_animation` | `0x430a70` | 57.88%, relationship-first | X-mesh animation owner that builds generated frame arrays from exact 0x80-byte `XAnimationKeyframe` records. | Pins borrowed keyframe object pointer `+0x24`, frame number `+0x7c`, retained animation pointer `Object +0xbc`, and the 0x14-byte animation / 8-byte frame graph. Three semantic interpolation stages recover native scheduling and prove the old swapped allocation-label conclusion was alignment fallout; residual is the candidate `0x3c` frame versus native `0x38`. |
| `replace_object_group_texture_refs` / `replace_object_list_texture_refs` / `select_level_track_texture_set` | `0x4145c0` / `0x430d90` / `0x410730` | 100% / 74.77% / 100% | Level texture-set selection rewrites texture refs across the global object list, keeping source facequads and already-built grouped render tables in sync. | Group-table helper is exact and pins `flags & 0x80000`, `Object +0x64`, and `Object +0xd0`. List helper now matches the native byte-cursor lifetime and loop-bottom schedule better; typed-array, direct texture-slot, simplified cursor, and absolute-address probes regress or stay neutral, leaving object-address register ownership as the main residual. Selector is exact after forcing the default case to reload the stack argument; keep it as a mutator because callers ignore the incidental `eax` result. |
| `rstrcpy_checked_ascii` / `strings_equal_case_insensitive` / `find_case_insensitive_substring` / `advance_to_next_crlf_line` / `strings_equal_case_insensitive_path` / `parse_next_float32` / `parse_next_signed_int` | `0x44e5b0` / `0x431dc0` / `0x44e600` / `0x44e690` / `0x44e6c0` / `0x431f20` / `0x44e710` | 100% / 84.00% / 85.25% / 100% / 100% / 63.57% / 100% | Shared resource string and parser helpers used by texture, object, X-mesh, level, segment, landscape, voice, and music loaders. | Copy, CRLF advance, path comparator, and signed-int parsing are exact. The signed-int scratch needs a `while (**cursor != '-')` pre-scan with the classifier byte read through the scoped cursor; the same idiom regresses the float parser by moving cursor ownership, so leave float on the retained `if` plus `do/while` shape. The substring helper now recovers native's pre-save searched-byte load and native null-before-success epilogue order, leaving the inner searched-byte reload tail as layout debt; equality now has the native left-byte loop and boolean tail, leaving only raw-before-fold byte-lane allocation debt. |
| `sample_tga_pixel_rgb` | `0x44e780` | 100% | Private font atlas helper used by `register_font_texture_sheet`; samples bottom-up TGA-like buffers and expands 8-bit grayscale or returns packed 24-bit RGB. | Exact after removing the named pixel pointer, using direct `image->bits_per_pixel` reads, and funneling both branches through `red`/`green`/`blue` locals; the grayscale branch assigns `blue`, `green`, then `red` to recover native's `ecx -> edi -> esi` copy chain. |
...
| `update_font_wave_state` / `font_slot_index_for_char` / `draw_font_text_instance` / `draw_queued_font_quad_instance` / `draw_font_text_queue` / `measure_font_text_width` / `queue_font_text_instance` / `queue_axis_aligned_textured_quad` / `queue_axis_aligned_textured_quad_uv` / `queue_textured_quad_corners` / `layout_and_queue_wrapped_font_text` / `register_font_texture_sheet` / `initialize_font3d_objects` | `0x449ca0` / `0x449d20` / `0x44a360` / `0x44a6d0` / `0x44a730` / `0x449e90` / `0x44a790` / `0x44a8b0` / `0x44a9b0` / `0x44aac0` / `0x44abe0` / `0x449f50` / `0x44ae10` | 100% / 100% / 35.67% / 100% / 100% / 22.86% / 61.84% / 69.70% / 76.12% / 64.29% / 94.19% / 72.46% / 44.34% | Shared font/textured-quad queue and atlas pipeline; pins the transient text buffer, queue cursor/count, render queue gate, `FontSheet` atlas metrics, `FontQueueEntry` text/quad/corner/color/UV/layer lanes, `TgaImageView` atlas sampling, and the non-sprite `Objects/Font3D` glyph-object consumer. | Exact wave reset, glyph mapper, queued-quad draw forwarder, queue drain, and `draw_galaxy_line` consumer are proof-grade; text draw/layout/atlas/3D-object producers are relationship-first with documented register/prologue, atlas-scan, stack-local, direct initial atlas reads, wrapped-text single-cursor layout, object-call reloads, and cursor residuals. `initialize_font3d_objects` now borrows a typed global `ObjectList` slot and uses the shared `Object`/`ObjectFaceQuad` layout, correcting `+0x14` from a font-local `ready` guess to `blend_mode` without changing code shape. `register_font_texture_sheet` leaves slot-count/font-kind tail ownership debt. |
| `populate_runtime_track_cells_from_segments` | `0x435eb0` | 28.25%, structure-first | Runtime glyph normalization, digit-on-attachment voiding, row ownership, attachment-entry row spans, cell anchors/UVs. | `P`/`p` entry install and shared post-switch anchor/color/UV/sub-object placement are landed. Next source-shape work is the switch physical order and the remaining `0x40` vs native `0x44` frame gap, without padding. |
| `mark_track_warning_zones` | `0x4354f0` | 36.27%, pinned | Warning footprint and hazard suppression. | Semantics are pinned; tile seed set, 6x2 footprint, bounds, and register-layout residuals are documented in NOTES. |
| `build_track_fringe_objects` | `0x434be0` | 60.39%, structure-first | Allocates directional fringe objects after runtime-cell build; `SubgameRuntime` owns the fixed 7,000-object manager while row cells retain borrowed handles. | Bool-result normalization now matches native. Remaining debt is register scheduling: native keeps game in `ebp`, family in `edi`, edge-a in `ebx`, then reuses/restores `ebp` for edge-b; skirt-color copy scheduling is documented in NOTES. |
| `is_neighbor_cell_solid` | `0x434b60` | 100% | Shared fringe-build predicate for lateral/row neighbor solidity, open-neighbor tile families, and tile-id exclusions. | Done; use as the exact fringe-build predicate for lateral/row neighbor solidity and open-neighbor tile-family checks. |
...
| `add_time_trial_high_score` | `0x4178b0` | 89.80%, source-shaped | Time Trial result persistence helper called by `complete_subgame`; owns scratch copy versus persistent route-record replacement. | Delayed route-offset shape is retained; remaining residual is `add ebp, ecx` versus folded compare addressing, with typed-pointer regressions documented in NOTES. |
...
| `add_arcade_high_score` / `add_survival_high_score` | `0x4176a0` / `0x417780` | 100% / 94.67%, source-shaped | Postal/Arcade and Challenge/Survival result persistence helpers called by `complete_subgame`; own top-ten insertion and high-score entry frontend arming. | Shared `HighScoreBank` layout pins postal, survival, time-trial route, current-result, and survival-pending record windows. `HighScoreRecord` now has a 0x14-byte `player_name`, `runtime_build_seed +0x68`, `replay_sample_count +0x6c`, and 21600 replay records at `+0x70` with `lateral_x`, `delta_z`, and flags. Arcade is proof-grade with the retained pre-tested down-shift loop; survival now keeps native prologue ownership with the bounded `int*` score scan, redundant guarded insertion, and post-shift saved-owner reload. Residuals are source-record reload scheduling and impossible-path frontend return handling; do not force with raw offset macros. |
...
| `update_subgame` | `0x438b90` | 78.22%, pinned | Outer gameplay state machine, active row scan, bridge opcodes. | States `0/1/2/3/4/7`, runtime rows, embedded `Player`, selected-level handoff, projected-ring lifetimes, time-trial record owner, and separate authored/procedural garbage arms recover the exact target instruction count with 116 clean operands. Remaining state-1/ring/HUD register scheduling and two jump-table label identities need new source evidence; do not force with long-lived aliases or fake control flow. |
...
| `orthogonalize_matrix` | `0x44d3d0` | 100% | Small basis repair helper that normalizes right/up/forward and rebuilds two axes through exact `cross_vectors`. | Done; use with exact `normalize_vector` and `cross_vectors` as the basis-repair source of truth. |
