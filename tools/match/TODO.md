...
| `update_subgame` | `0x438b90` | 70.97%, structure-first | Outer gameplay state machine, active row scan, bridge opcodes. | States `0/1/2/3/4/7`, runtime rows, and the embedded `Player` owner are covered. The selected-level handoff occupies the native cold tail, and projected-ring addresses now recover native short-lived ownership plus the shared last-Z store; remaining work is state-1 build/destroy scheduling, residual ring register allocation, and the localized time-trial record-base schedule. |
