...
| `sample_tga_pixel_rgb` | `0x44e780` | 25.29%, relationship-first | Private font atlas helper used by `register_font_texture_sheet`; samples bottom-up TGA-like buffers and expands 8-bit grayscale or returns packed 24-bit RGB. | Corrected the assumption that the whole `0x44e6xx` neighborhood was parser-only. `TgaImageView` layout and semantics are mapped; remaining residual is native image-pointer/register allocation and RGB packing shape documented in NOTES. |
| `update_font_wave_state` / `font_slot_index_for_char` / `draw_queued_font_quad_instance` / `draw_font_text_queue` / `measure_font_text_width` / `queue_font_text_instance` / `queue_axis_aligned_textured_quad` / `queue_axis_aligned_textured_quad_uv` | `0x449ca0` / `0x449d20` / `0x44a6d0` / `0x44a730` / `0x449e90` / `0x44a790` / `0x44a8b0` / `0x44a9b0` | 100% / 100% / 100% / 100% / 22.86% / 61.84% / 69.70% / 76.12% | Shared font/textured-quad queue pipeline; pins the transient text buffer, queue cursor/count, render queue gate, `FontSheet` atlas metrics, and `FontQueueEntry` text/quad/color/UV/layer lanes. | Exact wave reset, glyph mapper, queued-quad draw forwarder, and queue drain are proof-grade; width/text/quad producers are relationship-first with documented register/prologue, shared-zero epilogue, and queue-store scheduling residuals. Next useful slice is `draw_font_text_instance`, `queue_textured_quad_corners`, or `register_font_texture_sheet` against this shared layout. |
