...
| `populate_runtime_track_cells_from_segments` | `0x435eb0` | 18.90%, structure-first | Runtime glyph normalization, digit-on-attachment voiding, row ownership. | `P`/`p` attachment-entry install is decoded but not kept; land it together with the shared post-switch anchor to avoid regressing switch-region alignment. |
