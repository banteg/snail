...
| `create_golb` | `0x415280` | 27.93%, structure complete | Seeds projectile kind, velocity/spread, homing target, and RNG stream consumers. | Sprite-position copy now uses whole-vector source shape; continue around movement-flag branch sharing, direction/previous-output copy staging, and kind-specific setup locals documented in NOTES. |
