...
| `spawn_track_ring_or_special_effect` | `0x43df10` | 51.23%, evidence-first | Parent allocator/spawner for track ring and special-effect records; seeds parent position, active phase, active phase step, owner context, active list, and child particles. | Scan loop and staged parent-position copy now match native shape; remaining residual is grouped switch paths versus native per-kind RNG tags (`RR..RR13`) documented in NOTES. |
