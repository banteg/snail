...
| `refresh_object_vertex_buffer` | `0x412250` | 59.85%, relationship-first | Private `render_object` helper that refreshes grouped D3D vertex-buffer positions, animation frame vertex/normal sources, optional distortion, and static facequad UVs. | Field relationships are pinned: animation at `+0xbc`, render buffers at `+0xc0`, grouped vertex count at `+0xc4`, facequads at `+0x5c`, normals at `+0x60`, and distortion call base at `+0x80`. Remaining residual is native offset-loop register ownership; use this before expanding `render_object` or consolidating object render fields. |
