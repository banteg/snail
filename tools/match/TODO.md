...
| `spawn_track_garbage_hazard` | `0x43da80` | 99.30%, pinned | Seeds garbage scale, sprite variant, projected body position, active list link, and slot owner. Recent port fix depends on this path. | Semantics are pinned; remaining projection staging debt is one `fadd` scheduling residual, with rejected z-lane/vector-copy variants documented in NOTES. |
