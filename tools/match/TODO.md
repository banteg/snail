...
| `calc_path_length_z` | `0x4217b0` | 35.76%, structure complete | Golb path-follow riding; sibling of `update_track_attachment_follow_state`. | Terminal/side-exit copies, side-exit compare width, and matrix position zero-store order now match native better; continue source-shaping around overflow block placement, scalar lerp stack locals, and remaining matrix-copy layout documented in NOTES. |
