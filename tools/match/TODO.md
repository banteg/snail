...
|---|---:|---|---|---|
| `update_garbage_hazard` | `0x43f200` | 75.56%, source-shaped | Owns garbage active/burst states, smoke cadence, nuke reaction, and the final live body position that collision/projectile probes sample. | Signed push and burst cadence staging now match native arithmetic/store order; continue only around the `0x1c` frame gap, random velocity stack slots, side-bias compare/reload shape, and case-1 vector-copy register order documented in NOTES. |
