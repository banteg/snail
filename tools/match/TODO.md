...
| `update_subgame` | `0x438b90` | 67.53%, structure-first | Outer gameplay state machine, active row scan, bridge opcodes. | States `0/1/2/3/4/7`, selected-level overlay, timer/tutorial/voice gates, row-cell insertion paths, and bridge tile lanes are covered. The complete embedded `Player` owner at `+0x3bb764..+0x3bfac8` is now pinned; remaining work is state-1 build/destroy scheduling, state-2 bridge layout, and HUD/camera tail topology. |
