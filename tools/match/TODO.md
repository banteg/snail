...
| `update_font_wave_state` / `font_slot_index_for_char` / `measure_font_text_width` / `queue_axis_aligned_textured_quad` / `queue_axis_aligned_textured_quad_uv` | `0x449ca0` / `0x449d20` / `0x449e90` / `0x44a8b0` / `0x44a9b0` | 100% / 100% / 22.86% / 69.70% / 76.12% | Shared font/textured-quad queue pipeline; pins the transient text buffer, queue cursor/count, `FontSheet` atlas metrics, and `FontQueueEntry` stride/UV/color/layer lanes. | Exact wave reset and glyph mapper are proof-grade; width and quad helpers are relationship-first with documented register/prologue, shared-zero epilogue, and queue-store scheduling residuals. Next useful slice is `queue_font_text_instance`, `draw_font_text_instance`, or `register_font_texture_sheet` against this shared layout. |
