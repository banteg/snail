...
| `rstrcpy_checked_ascii` / `strings_equal_case_insensitive` / `find_case_insensitive_substring` / `advance_to_next_crlf_line` / `strings_equal_case_insensitive_path` / `parse_next_signed_int` | `0x44e5b0` / `0x431dc0` / `0x44e600` / `0x44e690` / `0x44e6c0` / `0x44e710` | 100% / 66.02% / 65.00% / 100% / 100% / 32.99% | Shared resource string and parser helpers used by texture, object, X-mesh, level, segment, landscape, voice, and music loaders. | Copy, CRLF advance, and path comparator are exact. Equality/substr/parser partials have current relationship-first shapes; native-loop and decompiler cursor rewrites regress or stay neutral, leaving register/control-flow residuals documented in NOTES. |
