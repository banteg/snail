...
| `update_subgame` | `0x438b90` | 78.22%, structure-first | Outer gameplay state machine, active row scan, bridge opcodes. | States `0/1/2/3/4/7`, runtime rows, and the embedded `Player` owner are covered. The selected-level handoff, projected-ring lifetimes, time-trial record owner, and separate authored/procedural garbage arms now recover the exact target instruction count; remaining work is state-1 build/destroy plus residual ring/HUD register scheduling. |
