...
| `update_subgoldy_bullet` | `0x43e830` | 71.41%, source-shaped | Parent virtual updater for ring/special-effect records: cadence, normal orbit, collapse/expand transitions, active-list removal, and child sprite teardown. | State-3 collapse now uses a real `Vector3 delta` and immutable target copy; remaining residual is direct 0..5 switch shape plus native's larger stack-temp frame documented in NOTES. |
