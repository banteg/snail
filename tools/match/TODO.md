...
|---|---:|---|---|---|
| `update_garbage_hazard` | `0x43f200` | 75.56%, source-shaped | Owns garbage active/burst states, smoke cadence, nuke reaction, and the final live body position that collision/projectile probes sample. | Signed push staging now matches native arithmetic/store order; continue only around the `0x1c` frame gap, random velocity stack slots, side-bias compare/reload shape, and case-1 vector-copy register order documented in NOTES. |
...
| `update_golb_ai` | `0x414820` | 21.84%, structure complete | Projectile flight, path entry, homing, trails, slug/garbage hit gates, wall impact. | Homing blend now stages pull/keep terms; continue around trail/smoke offset staging, direction update locals, slug deflection, and duplicated returns documented in NOTES. |
| `calc_path_length_z` | `0x4217b0` | 35.76%, structure complete | Golb path-follow riding; sibling of `update_track_attachment_follow_state`. | Terminal/side-exit copies and side-exit compare width now match native better; continue source-shaping around overflow block placement, scalar lerp stack locals, and matrix-copy layout documented in NOTES. |
| `create_golb` | `0x415280` | 28.54%, structure complete | Seeds projectile kind, velocity/spread, homing target, and RNG stream consumers. | Pointer lifetimes, movement-flag reloads, and previous-output whole-copy now better match native setup; continue around movement-flag branch sharing, direction copy staging, and kind-specific setup locals documented in NOTES. |
