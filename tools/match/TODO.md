...
| `rstrcpy_checked_ascii` / `strings_equal_case_insensitive` / `find_case_insensitive_substring` / `advance_to_next_crlf_line` / `strings_equal_case_insensitive_path` / `parse_next_float32` / `parse_next_signed_int` | `0x44e5b0` / `0x431dc0` / `0x44e600` / `0x44e690` / `0x44e6c0` / `0x431f20` / `0x44e710` | 100% / 66.02% / 69.29% / 100% / 100% / 63.57% / 70.10% | Shared resource string and parser helpers used by texture, object, X-mesh, level, segment, landscape, voice, and music loaders. | Copy, CRLF advance, and path comparator are exact. The substring helper now makes the native searched-string-ended-before-pattern path explicit, improving the inner termination shape; remaining residual is the pre-prologue searched-byte load and branchless null/success return selection. Equality residuals still look like byte-lane allocation/test-order debt. The numeric parser residual remains the native first-load gate plus decimal-loop/register scheduling documented in NOTES. |
