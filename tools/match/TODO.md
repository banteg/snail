...
| `add_arcade_high_score` / `add_survival_high_score` | `0x4176a0` / `0x417780` | 65.69% / 66.67%, source-shaped | Postal/Arcade and Challenge/Survival result persistence helpers called by `complete_subgame`; own top-ten insertion and high-score entry frontend arming. | New shared `HighScoreBank` layout pins postal, survival, time-trial route, current-result, and survival-pending record windows. Residuals are rank-scan register ownership and frontend global-store scheduling; do not force with raw offset macros. |
