...
| `spawn_track_garbage_hazard` | `0x43da80` | 82.69%, source-shaped | Seeds garbage scale, sprite variant, projected body position, active list link, and slot owner. Recent port fix depends on this path. | Pool-scan memory compare now matches native loop shape; continue source-shaping around projection x87/register order and setup register timing. |
