...
| `create_golb` | `0x415280` | 36.08%, structure complete | Seeds projectile kind, velocity/spread, homing target, and RNG stream consumers. | The attachment-family selector is now correctly owned as an optional vapour z-floor pointer, corroborated by Windows and iOS implementations; owner, velocity, sprite-slot, and follow-record lifetimes are typed. Continue only from concrete evidence around the remaining shared movement labels and kind-specific setup scheduling documented in NOTES. |
