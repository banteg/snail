...
| `update_golb_ai` | `0x414820` | 24.53%, structure complete | Projectile flight, path entry, homing, trails, slug/garbage hit gates, wall impact. | Live-state gate, path-follow output copies, non-follow position pointer staging, and direction delta locals now better match native; continue around the `0x70` frame gap, trail/smoke offset staging, slug deflection, and duplicated returns documented in NOTES. |
