...
| `spawn_track_garbage_hazard` | `0x43da80` | 75.52%, source-shaped | Seeds garbage scale, sprite variant, projected body position, active list link, and slot owner. Recent port fix depends on this path. | Continue source-shaping around pool-scan memory compare and projection x87/register order; semantics are documented in NOTES. |
