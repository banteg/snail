...
| `update_font_wave_state` / `font_slot_index_for_char` / `draw_queued_font_quad_instance` / `draw_font_text_queue` / `measure_font_text_width` / `queue_font_text_instance` / `queue_axis_aligned_textured_quad` / `queue_axis_aligned_textured_quad_uv` / `queue_textured_quad_corners` | `0x449ca0` / `0x449d20` / `0x44a6d0` / `0x44a730` / `0x449e90` / `0x44a790` / `0x44a8b0` / `0x44a9b0` / `0x44aac0` | 100% / 100% / 100% / 100% / 22.86% / 61.84% / 69.70% / 76.12% / 64.29% | Shared font/textured-quad queue pipeline; pins the transient text buffer, queue cursor/count, render queue gate, `FontSheet` atlas metrics, and `FontQueueEntry` text/quad/corner/color/UV/layer lanes. | Exact wave reset, glyph mapper, queued-quad draw forwarder, and queue drain are proof-grade; width/text/quad producers are relationship-first with documented register/prologue, shared-zero epilogue, queue-store scheduling, and unused stack-slot residuals. Next useful slice is `draw_font_text_instance` or `register_font_texture_sheet` against this shared layout. |
