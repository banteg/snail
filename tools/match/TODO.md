...
| `update_subgame` | `0x438b90` | 69.45%, structure-first | Outer gameplay state machine, active row scan, bridge opcodes. | States `0/1/2/3/4/7`, runtime rows, and the embedded `Player` owner are covered. The selected-level handoff now occupies the native cold tail with exact per-arm state snapshots; remaining work is state-1 build/destroy scheduling, ring exits, and the localized time-trial record-base schedule. |
