...
|---|---:|---|---|---|
| `update_garbage_hazard` | `0x43f200` | 51.48%, structure complete | Owns garbage active/burst states, smoke cadence, nuke reaction, and the final live body position that collision/projectile probes sample. | Continue source-shaping around local frame, burst float staging, and vector copy register order; semantics are documented in NOTES. |
...
| `spawn_track_garbage_hazard` | `0x43da80` | 63.38%, structure complete | Seeds garbage scale, sprite variant, projected body position, active list link, and slot owner. Recent port fix depends on this path. | Continue source-shaping around local frame/projection staging; semantics are documented in NOTES. |
