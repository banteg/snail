// Authored cRToolTip owner embedded in FrontendWidget at +0x28c. The widget
...
public:
    ...
    FrontendWidget* owner_widget; // +0x0c, borrowed enclosing widget backlink
    float delay_progress; // +0x10
    float delay_step; // +0x14
    FrontendWidget* tooltip_widget; // +0x18, BorderManager-owned live widget handle
    char unknown_01c[0x38 - 0x1c];
    FrontendWidget* owner_widget_38; // +0x38, second borrowed widget backlink
    char unknown_03c[0x40 - 0x03c];
};

typedef char FrontendWidgetTooltip_must_be_0x40[
    (sizeof(FrontendWidgetTooltip) == 0x40) ? 1 : -1];
