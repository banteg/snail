...
- Exit opens the completion/exit prompt path by saving the previous front-end
  state, copying the exit widget layout y into `cRExit::prompt_y`,
  setting prompt state `10`, and switching front-end state to `8`.
- Options raises `cROptions::active`, records the previous state, and switches
  to state `6`.
...
- 2026-07-11 owner closure promotes the contiguous root block to
  `Intro`, `cRMainMenu`, `cRStarManager`, `cROptions`, and
  `cRExit`; this remains exact 68/68.
...
`cRExit`, `cROptions`, and `HighScore` members. Native's callback-separated root
...
`cRMainMenu` receiver and the adjacent `cROptions`/`cRExit` root fields. The tracked
