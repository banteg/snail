...
Current source maps the native behavior: validates the live flag, reports the
texture-name-backed error string from `texture_ref->name` (`TextureRef+0x0c`),
unlinks from the owner active bucket, and pushes the sprite onto
`g_sprite_manager.free_head`.
