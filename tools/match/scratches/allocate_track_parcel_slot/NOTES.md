...
`cRParcelManager::New()` scans its 50 inline `cRParcel`
...
active. Android independently retains this member, and the stable matcher key
binds `?New@cRParcelManager@@QAEPAVcRParcel@@XZ`.
...
2026-06-16 layout assertion pass: `track_parcel_runtime.h` now asserts both the
primary `cRParcel == 0x8c` / `cRParcelManager == 0x1b58` owners and their
...
2026-07-14 traversal ownership: the state scan now advances a typed
`cRParcel*` through `cRParcelManager::slots` instead of adding the 0x23-word representation
...
replays keep the stable `Parcel` / `ParcelManager` names and refuse width drift;
the matcher uses the authored `cRParcel` / `cRParcelManager` surface. Matching
remains exact at 18/18 instructions with both operands clean.
