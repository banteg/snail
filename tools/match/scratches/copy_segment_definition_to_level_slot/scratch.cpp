...
struct AuthoredSegmentRow {
    ...
    int object_rotation_x_bits;    // +0x24
    int object_rotation_y_bits;    // +0x28
    int object_rotation_z_bits;    // +0x2c
    int attachment_template_index; // +0x30
    int row_event_id;              // +0x34
};
...
int* __stdcall copy_segment_definition_to_level_slot(char* segment_name, LevelSegmentSlot* slot)
{
    ...
    while (index < *catalog_count) {
        if (strings_equal_case_insensitive_path(segment_name, scan->name) == 1)
            break;
        ++index;
        ++scan;
    }
    ...
    slot->source_name = source->name;
    slot->active_row_base = 0;
    slot->path_index = source->path_index;
    ...
    for (int metadata_row = 0; metadata_row < source->row_count; ++metadata_row) {
        ...
        destination->object_rotation_x_bits = authored->object_rotation_x_bits;
        destination->object_rotation_y_bits = authored->object_rotation_y_bits;
        destination->object_rotation_z_bits = authored->object_rotation_z_bits;
        ...
        destination->attachment_template_index = authored->attachment_template_index;
        destination->row_event_id = authored->row_event_id;
