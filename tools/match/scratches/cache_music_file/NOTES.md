...
- Focused Wibo result: 2026-06-18, 100.00%, 42/42 instructions, masked
  operands 11 ok.
- Front-end callers push `(path, 0, g_blank_text)` and clean 12 bytes after the
  call; the helper only consumes `path`, so the remaining two source parameters
  are unused/default slots rather than separate behavior.
