...
- The penultimate integer selects the immediate blend/render-state helper at
  `sub_412e50`.
...
- prefix: 111 / 332
- masked operands: 24 clean, 1 unresolved, 0 mismatched

Remaining gap:

The reconstruction is a near match. The unresolved operand is the call to the
local immediate render-state helper `sub_412e50`. The remaining instruction
differences are minor jump-label displacement churn, one x87 `fst` versus
`fld st(0)` scheduling choice while computing the rotated radius, one load/store
ordering difference in the rotated vertex block, and the success-counter tail:
MSVC materializes the source increment as load/inc/store because the scratch
uses a normal defined `return result`, while native leaves `eax` live and emits
`inc dword [data_4f7454]`.
