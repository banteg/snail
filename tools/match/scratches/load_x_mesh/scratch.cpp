...
void DirectXLoader::load_x_mesh(char* mesh_path, Object* object, int options_flags)
{
    ...
    char* frame_cursor = Rstrfind("Frame ", file_text);
    if (frame_cursor == 0) {
    ...
    char* duplicate_cursor = Rstrfind("Mesh ", frame_cursor);
    if (duplicate_cursor == 0)
        report_errorf("No 'VertexDuplicationIndices 'Data in %s", mesh_file_path);
    duplicate_cursor = Rstrfind("{", duplicate_cursor);

    char* mesh_cursor = Rstrfind("Mesh ", frame_cursor);
    if (mesh_cursor == 0)
        report_errorf("No 'Mesh 'Data in %s", mesh_file_path);
    mesh_cursor = Rstrfind("{", mesh_cursor);

    char* material_header_cursor = Rstrfind("MeshMaterialList ", frame_cursor);
    if (material_header_cursor == 0)
        report_errorf("No 'MeshMaterialList 'Data in %s", mesh_file_path);
    material_header_cursor = Rstrfind("{", material_header_cursor);

    Rstrint(&material_header_cursor);
    short parsed_facequad_count = (short)Rstrint(&material_header_cursor);
    int parsed_vertex_count = Rstrint(&duplicate_cursor);
    int vertex_count = (short)parsed_vertex_count;
    if (vertex_count != Rstrint(&mesh_cursor))
        report_errorf(
            "Mesh vertices count does not match vertext duplicate vertices count in %s",
            mesh_file_path);
    Rstrint(&duplicate_cursor);

    char* texcoord_cursor = Rstrfind("MeshTextureCoords ", frame_cursor);
    if (texcoord_cursor == 0)
        report_errorf("No 'Mesh 'Data in %s", mesh_file_path);
    texcoord_cursor = Rstrfind("{", texcoord_cursor);
    if (vertex_count != Rstrint(&texcoord_cursor))
        report_errorf(
    ...
    if (facequad_count != Rstrint(&mesh_cursor))
    ...
    for (i = 0; i < facequad_count; ++i) {
        ...
        int arity = Rstrint(&mesh_cursor);
        int vertex_2 = Rstrint(&mesh_cursor);
        int vertex_1 = Rstrint(&mesh_cursor);
        int vertex_0 = Rstrint(&mesh_cursor);
        int vertex_3;
        if (arity == 4) {
            vertex_3 = Rstrint(&mesh_cursor);
        } else {
            vertex_3 = 0;
            object->facequads[i].header_word |=
                OBJECT_FACEQUAD_FLAG_TRIANGLE;
        }
    ...
    char* material_cursor = Rstrfind("MeshMaterialList", frame_start);
    if (material_cursor == 0) {
        report_errorf("No MeshMaterialList { in %s", mesh_file_path);
        return;
    }

    material_cursor = Rstrfind("{", material_cursor);
    int material_count = Rstrint(&material_cursor);
    int material_face_count = Rstrint(&material_cursor);
    ...
    if (material_count > 0) {
        ...
        do {
            char* texture_name =
                Rstrfind("TextureFilename ", material_text);
            if (texture_name == 0) {
                ...
                *material_slot =
                ...
                material_text = Rstrfind("\"", texture_name) + 1;
                texture_path[0] = 'X';
                texture_path[1] = '/';
    ...
    for (i = 0; i < material_face_count; ++i) {
        object->facequads[i].texture_ref =
            materials[(short)Rstrint(&material_cursor)];
    }
