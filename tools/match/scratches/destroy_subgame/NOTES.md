...
- if the level mode is tutorial (`7`), calls
  `cRTutorial::UnInit()` at `game+0xa858`;
...
Tutorial-mode teardown likewise reaches the primary embedded `cRTutorial`.
...
The opening teardown write at root `+0x4f26c` is also inside the exact
`GameRoot::backdrop` extent, at `Backdrop +0x65c`. Android's corresponding
member is the four-corner index-buffer handle generated by
`cRBackdrop::MakeVBO()`, so the shared field is now named
`corner_index_buffer_handle`. The Windows backend only initializes and resets
the slot; it does not otherwise consume it.
