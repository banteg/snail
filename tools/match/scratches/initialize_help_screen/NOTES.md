...
`cRHelp`, and this initializer remains exact at 47/47.
...
authored `void cRHelp::Init()` ABI and exact four-byte `cRHelp` owner. The
tracked output reaches `help->back_button` directly instead of treating the
owner as a loose `FrontendWidget**`; the replay rejects any `cRHelp` size other
than `0x04`.

## 2026-08-12 authored method surface

The scratch now spells the exact member as `cRHelp::Init()` and exports
`?Init@cRHelp@@QAEXXZ`. Its sole live Windows caller passes the four-byte cRHelp
...
The generic analysis-only `Help` record is now migrated to canonical `cRHelp`
after exact four-byte layout equivalence. Focused replay verifies its sole Back
widget plus the adjacent `cRGUI` and `cRSplash` boundaries without changing the
already exact matching source.
