// Coupled receiver/parameter lifetime probe for add_object_edge @ 0x4305a0.
...
{
    cRObject* object = this;
    int normal_index = normal_index_argument;
    Vector3 direction;
    if (object->facequad_normals[normal_index].Magnitude() < 0.89999998f) {
        return;
    }
    ...
    if (build_count > 0) {
        ...
        Vector3* vertex_b_position = &object->vertices[vertex_b];
        do {
            Vector3* edge_a_position =
                &object->vertices[edge_vertex_b[-1]];
            if (edge_a_position->x == vertex_b_position->x
                && edge_a_position->y == vertex_b_position->y
                && edge_a_position->z == vertex_b_position->z) {
                Vector3* edge_b_position =
                    &object->vertices[*edge_vertex_b];
                Vector3* vertex_a_position =
                    &object->vertices[vertex_a];
    ...
    if (found_edge == 0) {
        ...
        Vector3* start = &object->vertices[vertex_a];
        Vector3* end = &object->vertices[vertex_b];
        Vector3 edge_delta;
    ...
    if ((g_object_edge_build_edges[index].flags
        ...
        if ((object->flags & OBJECT_FLAG_DYNAMIC_VERTICES) == 0) {
            Vector3 lhs = object->facequad_normals[
                g_object_edge_build_edges[index].normal_a];
            Vector3 rhs = object->facequad_normals[
                g_object_edge_build_edges[index].normal_b];
            Vector3 cross;
