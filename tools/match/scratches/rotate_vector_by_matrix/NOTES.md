...
- `tVector::Rotate` now has only the exact
...
- Focused rechecks stayed at their baselines: this helper 100.00%,
  ...
  `calc_path_length_z` 69.41%, `build_sprite_tail` 97.67%, and
  `render_object_toon` 57.76%. The former header-only vector ABI-conflict row is
