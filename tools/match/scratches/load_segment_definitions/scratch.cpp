...
void cRSMTracks::Import() {
    ...
    count = 0;
    enumerate_matching_archive_or_fs_entries("Segments", "*.txt", &count,
                                             segment_files[0]);
    ...
    while (segment_index < count) {
        ...
        short row_index = 0;
        entries[segment_index].row_count = 0;
        while (data_cursor[0] != '@' || data_cursor[1] != '@' ||
               data_cursor[2] != '@') {
            entries[segment_index].rows[row_index].flags = 0;
            ++data_cursor;

            int lane = 0;
            do {
                entries[segment_index]
                    .glyph_columns[entries[segment_index].row_count][lane] =
                    *data_cursor++;
                ++lane;
            } while (lane < 8);
            ...
            ++entries[segment_index].row_count;

            if (*++data_cursor == '*')
                entries[segment_index].rows[row_index].flags |=
                    AUTHORED_SEGMENT_ROW_FLAG_SUPPRESS_TRACK_RENDER;

            char* option_out = option_text;
            char option_char = *data_cursor;
            while (option_char != '\r') {
                *option_out++ = option_char;
                ++data_cursor;
                option_char = *data_cursor;
            }
            *option_out = 0;

            char* option_match = Rstrfind("3DModel=", option_text);
            if (option_match != 0) {
                ...
                *mesh_out++ = 'x';
                *mesh_out = 0;
                entries[segment_index].rows[row_index].object_id =
                    g_game->directx_loader.ModelAdd(mesh_name);

                option_match = Rstrfind("(", option_match);
                entries[segment_index].rows[row_index].object_position.x =
                    RTextExtractFloat(&option_match);
                entries[segment_index].rows[row_index].object_position.y =
                    RTextExtractFloat(&option_match);
                entries[segment_index].rows[row_index].object_position.z =
                    RTextExtractFloat(&option_match);
                ...
                if (option_match != 0) {
                    ...
                    option_match = Rstrfind("(", option_match);
                    entries[segment_index].rows[row_index].object_velocity.x =
                        RTextExtractFloat(&option_match);
                    entries[segment_index].rows[row_index].object_velocity.y =
                        RTextExtractFloat(&option_match);
                    entries[segment_index].rows[row_index].object_velocity.z =
                        RTextExtractFloat(&option_match);
                }
            }
            ...
            if (option_match != 0) {
                ...
                option_match = Rstrfind("=", option_match) + 1;
                entries[segment_index].rows[row_index].parcel_set_id =
                    Rstrint(&option_match);
                option_match = Rstrfind("(", option_match) + 1;
                entries[segment_index].rows[row_index].local_position.x =
                    RTextExtractFloat(&option_match);
                entries[segment_index].rows[row_index].local_position.y =
                    RTextExtractFloat(&option_match);
                entries[segment_index].rows[row_index].local_position.z =
                    RTextExtractFloat(&option_match);
            }
            ...
            if (option_match != 0) {
                ...
                *path_out = 0;
                entries[segment_index].rows[row_index].path_template_index =
                    g_game->subgame.path_manager.NameCode(path_name);
                if (entries[segment_index].rows[row_index].path_template_index == -1)
                    report_errorf("Unknown path %s in %s", path_name,
                                  segment_files[segment_index]);
                else
                    entries[segment_index].rows[row_index].flags |=
                        AUTHORED_SEGMENT_ROW_FLAG_PATH_OR_MODEL_VELOCITY;
            ...
            if (option_match != 0) {
                option_match = Rstrfind("=", option_match) + 1;
                entries[segment_index].rows[row_index].ring_speed.value =
                    RTextExtractFloat(&option_match);
            } else {
                entries[segment_index].rows[row_index].ring_speed.bits = 0;
            }
