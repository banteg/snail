...
`tVector::operator*(const TransformMatrix&) const` affine transform retained by
...
the compatibility `Vector3` spelling in `tools/match/include/vector3.h`, and the three local math/Bod
...
At that point the remaining owner debt was isolated to the header-only rotate
overload split; the 2026-07-28 promotion now makes `tVector` primary.
