...
- Seeds every runtime cell's `lane_and_flags` with `0x6000`.
...
- Clears continuation-cell render/contact bit `0x20` and merge bits `0x6000`.
...
- Treating continuation cells as low-bit `0x40` gates was wrong; the native
  tests `0x4000` for run continuation after the seed pass.
