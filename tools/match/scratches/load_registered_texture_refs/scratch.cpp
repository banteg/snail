...
void load_registered_texture_ref(int texture_index, int debug_fallback); // @ 0x412a70
int debug_report_stub(char* format, ...); // @ 0x449c00, stripped in release

void load_registered_texture_refs(int debug_fallback)
{
    ...
    for (int index = 0; index < g_texture_refs.count; ++index) {
        load_registered_texture_ref(index, debug_fallback);
    }
